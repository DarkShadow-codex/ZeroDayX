...
ZeroDay can connect to [Model Context Protocol (MCP)](https://modelcontextprotocol.io) servers you list and expose their tools to the agent during a run. Use this to let the agent read how your system is actually built instead of inferring it from the outside.
...
Create the file `~/.zeroday/mcp-servers.json`. It holds a JSON list of the servers you want the agent to reach. Each entry is either a local `stdio` server that ZeroDay launches as a subprocess, or a remote `http` server.
...
Paste the servers you want into `~/.zeroday/mcp-servers.json`. The example below shows one of each transport: a local filesystem server over `stdio` and a remote GitHub server over `http` with a bearer token:
...
ZeroDay reads this file at the start of each run. There is no default file, so no MCP tools are loaded until you create it. Edit `command`, `args`, `url`, and `token` to match your own servers.
...
<ParamField path="command" type="string">
  For `stdio` servers: the executable ZeroDay launches (for example `npx`).
</ParamField>
...
<ParamField path="allowed_tools" type="array">
  ...
  server offers, or set it to a list of tool names to allow only those. ZeroDay
...
```bash
zeroday --mcp-server github -t ...        # use only the named connection(s)
zeroday --mcp-exclude staging-db -t ...   # use everything except the named one(s)
```
...
To read the config from another path instead of `~/.zeroday/mcp-servers.json`, either pass `--mcp-config <path>` on the command line:

```bash
zeroday --mcp-config ./mcp-servers.json -t ...
```

or set the `ZERODAY_MCP_CONFIG` environment variable to that path. The flag takes precedence when both are given.

## Startup confirmation

When servers are configured, ZeroDay prints a one-line summary at scan startup, for example `MCP: connected 1 server (14 tools): local_fs`, so you can confirm your servers connected.
...
viewer (`zeroday view`), so a call that left ZeroDay for a server you connected is
