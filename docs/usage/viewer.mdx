...
description: "Browse a run in a local dashboard with zeroday view"
---

Every scan writes its results to disk as it runs. `zeroday view` serves those files in a local dashboard, for a live run or a finished one.

```bash
zeroday view                                    # the most recent run
zeroday view my-run-name                        # a specific run under ./zeroday_runs
zeroday view --host 0.0.0.0 --port 8080 --no-open
```

The UI ships prebuilt with ZeroDay, so there is no extra install and no JavaScript build step. The dashboard reads the run files straight off disk. Nothing leaves your machine, and you do not need a cloud account.

## Options

<ParamField path="run" type="string">
  Run name under `./zeroday_runs`. Defaults to the most recent run.
</ParamField>
...
- **Steering** — send instructions to a live scan to redirect the agents during the run. Steering works only in the dashboard the running scan opens. A standalone `zeroday view` has no live scan to steer.
