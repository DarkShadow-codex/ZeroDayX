...
ZeroDay offers three scan modes to balance speed and thoroughness.

## Quick

```bash
zeroday --target ./app --scan-mode quick
```
...
```bash
zeroday --target ./app --scan-mode standard
```
...
```bash
zeroday --target ./app --scan-mode deep
```
