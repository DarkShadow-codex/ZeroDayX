---
title: "CLI Reference"
description: "Command-line options for ZeroDay"
---

## Basic Usage

```bash
zeroday (--target <target> | --target-list <path>) [options]
```
...
<ParamField path="--target, -t" type="string">
  ...
  When the target is an API spec, ZeroDay copies it into the agent's workspace and authorizes the base URLs it declares (including those resolved from a Postman environment) as in-scope hosts - so the agent reads the contract and tests the full declared surface instead of discovering endpoints by crawling. Pair the spec with the deployed base URL (e.g. `--target ./openapi.yaml --target https://api.example.com`) so the agent has a reachable host to attack.
...
<ParamField path="--config" type="string">
  Path to a custom config file (JSON) to use instead of `~/.zeroday/cli-config.json`.
</ParamField>
...
<ParamField path="--max-budget" type="number">
  ...
  **Limitations**
  ...
  - For LiteLLM-routed models, ZeroDay enables streaming success callbacks to
...
zeroday --target https://app.com --instruction "Use credentials: user:pass"

# Focused testing
zeroday --target api.example.com --instruction "Focus on IDOR and auth bypass"

# CI/CD mode
zeroday -n --target ./ --scan-mode quick

# Cap cost and per-agent turns
zeroday --target https://example.com --max-budget 25 --max-turns 300

# Force diff-scope against a specific base ref
zeroday -n --target ./ --scan-mode quick --scope-mode diff --diff-base origin/main

# Multi-target white-box testing
zeroday -t https://github.com/org/app -t https://staging.example.com

# API spec + live target (OpenAPI/Swagger file or Postman collection)
zeroday -t ./openapi.yaml -t https://api.example.com

# Postman collection pulled live by id (+ optional environment)
zeroday -t "postman://<collection-uuid>?env=<environment-uuid>"
