...
echo -e "${BLUE}🦉 ZeroDay Build Script${NC}"
echo "================================"
...
TUI_BINARY="build/sidecar/zeroday-tui"
...
uv run pyinstaller zeroday.spec --noconfirm

RELEASE_DIR="dist/release"
mkdir -p "$RELEASE_DIR"

BINARY_NAME="zeroday-${VERSION}-${OS_NAME}-${ARCH_NAME}"

if [ "$OS_NAME" = "windows" ]; then
    if [ ! -f "dist/zeroday.exe" ]; then
        echo -e "${RED}Build failed: Binary not found${NC}"
        exit 1
    fi
    BINARY_NAME="${BINARY_NAME}.exe"
    cp "dist/zeroday.exe" "$RELEASE_DIR/$BINARY_NAME"
    ...
    if [ ! -f "dist/zeroday" ]; then
        echo -e "${RED}Build failed: Binary not found${NC}"
        exit 1
    fi
    cp "dist/zeroday" "$RELEASE_DIR/$BINARY_NAME"
