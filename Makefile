...
TUI_BINARY := build/sidecar/zeroday-tui$(if $(filter Windows_NT,$(OS)),.exe)
...
type-check:
	...
	uv run pyright zeroday/
	@echo "✅ Type checking complete!"

security:
	@echo "🔒 Running security checks with bandit..."
	uv run bandit -r zeroday/ -c pyproject.toml
...
viewer:
	...
	cd zeroday/interface/viewer/frontend && npm ci && npm run build
	@echo "✅ Viewer built to zeroday/interface/viewer/static/ (commit the changes)."
...
tui-build:
	...
	cd zeroday/interface/tui && CGO_ENABLED=0 go build -trimpath -ldflags="-s -w" -o ../../../$(TUI_BINARY) ./cmd/zeroday-tui

tui-test:
	cd zeroday/interface/tui && go test -race ./...

tui-lint:
	cd zeroday/interface/tui && test -z "$$(gofmt -l .)" && go vet ./...
