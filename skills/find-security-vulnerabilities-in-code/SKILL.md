...
White-box security review with ZeroDay: the agents read the source to build a model of routes, sinks, and authorization checks, then attempt real exploitation. Findings come with a proof-of-concept, so the output is a short list of proven issues rather than the hundreds of "potential" hits a pattern-matching scanner produces.

Install, LLM setup, all flags, and the managed-cloud path are in the **penetration-testing-with-zeroday** skill. For a run with no Docker and no LLM key, the same binary drives the managed platform: `zeroday cloud login`, then `zeroday cloud scans start ...` (details in **managed-pentesting-with-zeroday**).

## Run it

```bash
# Local working tree
zeroday -n -t ./ --scan-mode standard --max-budget 15

# A GitHub repo directly
zeroday -n -t https://github.com/org/app --max-budget 15

# Monorepo: point at the service that matters, not the whole tree
zeroday -n -t ./services/checkout --max-budget 20

# Only what a branch changed (whole-repo review is wasteful on a large repo)
zeroday -n -t ./ --scope-mode diff --diff-base origin/main --max-budget 10
```
...
2. **Scope the review.** Point at the risky subtree and say what matters:
   ```bash
   zeroday -n -t ./services/api --max-budget 15 \
...
For diff-scoped review of a branch or PR (and blocking merges on findings), use **ci-security-scanning-with-zeroday** — it covers diff scoping, PR comments, and SARIF upload to GitHub code scanning. The managed platform can also review PRs directly via API (**managed-pentesting-with-zeroday**).

## Read the results

In `zeroday_runs/<run>/`: `penetration_test_report.md` (start here), `vulnerabilities/*.md` (one per finding, with PoC and remediation), `vulnerabilities.json` / `.csv`, `findings.sarif` (upload to code scanning), `run.json`.
...
Hand results to **fix-security-vulnerabilities-with-zeroday**: patch the root cause (the shared authorization helper, not the one route), then re-run ZeroDay to prove the exploit no longer works.
