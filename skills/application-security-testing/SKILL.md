...
| Every pull request, continuously | **ci-security-scanning-with-zeroday** |
| No Docker, no LLM key, or a report an auditor will accept | **managed-pentesting-with-zeroday** |
...
Findings arrive per run in `zeroday_runs/<run>/`. Merge them into a single list and rank by **proven impact**, not by scanner severity:
...
Then remediate with **fix-security-vulnerabilities-with-zeroday**, which re-runs ZeroDay against each fix to prove the exploit no longer works.
