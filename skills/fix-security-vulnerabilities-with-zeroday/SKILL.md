...
# Fix ZeroDay findings and verify

Turn validated ZeroDay findings into minimal, correct fixes — and prove they work by re-scanning.

## 1. Triage

Get the findings from wherever the scan ran:

- **OSS CLI** — artifacts in `zeroday_runs/<run-name>/`:
...
- **Cloud (app.zeroday.ai)** — pull findings with the CLI: `zeroday cloud vulns list --scan-id <scan-id> --json` (or `zeroday cloud scans get <scan-id> --json | jq '.vulnerabilities'`, or `zeroday cloud vulns list --severity critical` org-wide). Each finding carries `severity, cwe, endpoint, method, impact, technical_analysis, poc_description, poc_script_code` and, for code findings, `code_file`/`code_diff`/`code_before`/`code_after`. After a fix is verified, mark it with `zeroday cloud vulns update <id> --status fixed`. See the **managed-pentesting-with-zeroday** skill for `zeroday cloud login` and scopes.

Order work by severity: critical → high → medium → low. Every ZeroDay finding was validated with a working proof-of-concept, so do not dismiss findings as false positives without re-testing the PoC yourself.
...
## 3. Verify by re-running ZeroDay
