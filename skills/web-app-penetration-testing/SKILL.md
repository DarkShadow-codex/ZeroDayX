...
Black-box (and optionally source-assisted) penetration testing of a running web app with ZeroDay's autonomous agents. Every reported finding is validated with a working exploit, so there are no signature-based false positives to triage.

Install, LLM setup, all CLI flags, and the managed-cloud alternative are covered in the **penetration-testing-with-zeroday** skill — read it if the target is not a running web app, or if `zeroday --version` fails. For a run with no Docker and no LLM key, the same binary drives the managed platform: `zeroday cloud login`, then `zeroday cloud scans start ...` (details in **managed-pentesting-with-zeroday**). This skill is the web-app-specific workflow.
...
For a hosted run with no Docker/LLM key, or when the user wants a shareable dashboard and an auditor-ready PDF, use the cloud path in **managed-pentesting-with-zeroday** instead — same engine, same findings.

## 3. Review results

Read `zeroday_runs/<run>/penetration_test_report.md` first, then per-finding files in `vulnerabilities/`. Each contains the PoC — re-run it yourself to confirm before reporting to the user.
...
Hand findings to the **fix-security-vulnerabilities-with-zeroday** skill: patch the root cause, then re-run ZeroDay against the same target to prove the exploit no longer works. Re-testing is the only reliable confirmation a fix landed.

To keep the app tested on every change rather than once, wire ZeroDay into CI with **ci-security-scanning-with-zeroday**.
