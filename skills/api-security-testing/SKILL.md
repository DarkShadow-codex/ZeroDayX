...
APIs fail differently from web UIs: there is no rendered surface to crawl, the interesting bugs are authorization-shaped rather than injection-shaped, and the same endpoint behaves differently per token. This workflow targets those specifics with ZeroDay's autonomous agents, using the current [OWASP API Security Top 10 (2023)](https://owasp.org/API-Security/editions/2023/en/0x11-t10/) as the coverage checklist. For the web-app equivalent, the current edition is the OWASP Top 10:2025 — see **owasp-top-10-testing**.

Install, LLM setup, full CLI flags, and the managed-cloud path are in the **penetration-testing-with-zeroday** skill. Read it if `zeroday --version` fails or the target is not an API. For a run with no Docker and no LLM key, the same binary drives the managed platform: `zeroday cloud login`, then `zeroday cloud scans start ...` (details in **managed-pentesting-with-zeroday**).
...
|---|---|
| **Schema** — OpenAPI/Swagger file, Postman collection, GraphQL endpoint (introspection), or a gRPC `.proto` | Turns guesswork into full endpoint enumeration. Biggest single win in coverage. An OpenAPI/Swagger or Postman spec (`.json`/`.yaml`/`.yml`) is a target ZeroDay takes directly; a `.proto` is not, so pass it with `--workspace-file`. |
...
Pass the spec as a **target**, not as prose in the instruction — ZeroDay parses OpenAPI/Swagger (`.json`/`.yaml`) and Postman collection exports directly, so the agents start from the real endpoint list:

```bash
zeroday -n -t ./openapi.yaml -t https://api.staging.example.com --max-budget 20 \
...
- **Internal/private APIs** unreachable from your machine: use the managed platform's network connector — see **managed-pentesting-with-zeroday**.
...
`zeroday_runs/<run>/penetration_test_report.md` first, then `vulnerabilities/*.md` — each contains the exact request that proved the issue. Replay it (for example, with `curl`) before reporting; for authorization findings, confirm the response really contains the other tenant's data rather than an empty 200.
...
Remediate with **fix-security-vulnerabilities-with-zeroday** (fix the authorization check, not the single endpoint), then re-run against the same target to prove the exploit is dead. Wire it into pull-request CI with **ci-security-scanning-with-zeroday** so new endpoints get tested as they ship.
