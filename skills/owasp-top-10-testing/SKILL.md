...
Install, LLM setup, and the managed-cloud alternative: **penetration-testing-with-zeroday**. For a run with no Docker and no LLM key, the same binary drives the managed platform: `zeroday cloud login`, then `zeroday cloud scans start ...` (details in **managed-pentesting-with-zeroday**).
...
- Need an auditor-facing PDF? Run it through the managed platform and pull the technical report (**managed-pentesting-with-zeroday**).

## Report honestly

From `zeroday_runs/<run>/`, group `vulnerabilities/*.md` by category and state, per category: what was attempted, what was proven, and what could not be assessed (A09 always; A03/A04/A06/A08/A10 partially). Label the report with the edition used. Verify each PoC yourself before it goes in front of the user.
...
Remediate with **fix-security-vulnerabilities-with-zeroday** and re-run to prove each exploit is closed. For ongoing coverage as the app changes, gate pull requests using **ci-security-scanning-with-zeroday**.
