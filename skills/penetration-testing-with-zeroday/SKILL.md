---
name: penetration-testing-with-zeroday
description: Pentest a web app, API, codebase, repository, URL, domain, or IP with ZeroDay — autonomous AI penetration testing that exploits and proves vulnerabilities (OWASP Top 10 and beyond — injection, XSS, SSRF, auth/access-control flaws, IDOR, business logic) instead of just flagging them. Runs self-hosted with the open-source CLI or via the managed app.zeroday.ai cloud, and returns validated findings with proof-of-concept exploits (Markdown, JSON, CSV, SARIF). Use when the user asks to pentest, hack, security-scan, security-audit, or find vulnerabilities in an app, API, website, or repo.
metadata:
  author: ZeroDay
  homepage: https://docs.zeroday.ai
---

# Run a ZeroDay pentest

ZeroDay runs autonomous AI pentesting agents that dynamically exploit a target and only report findings validated with a working proof-of-concept. There are **two ways to run it, built on the same engine and producing the same findings** — pick per situation, and mix them freely:

- **Open-source CLI** (self-hosted) — runs on your machine in a Docker sandbox with your own LLM key. Free, fully local, BYO-LLM, air-gap capable. Docs: [docs.zeroday.ai](https://docs.zeroday.ai).
- **Managed cloud** — runs on ZeroDay's infrastructure, driven from the same CLI (`zeroday cloud ...`) or the REST API at `https://app.zeroday.ai/api/v1`. No Docker, no LLM key, no local compute; adds team dashboards, scheduling, PR reviews, downloadable PDF/DOCX reports (Enterprise plan), and internal-network connectors. Docs: [docs.app.zeroday.ai](https://docs.app.zeroday.ai). Full workflow in the **managed-pentesting-with-zeroday** skill.
...
If unsure and the user has (or will create) an app.zeroday.ai account, prefer **Cloud** — it avoids all local-infra friction. If they want zero signup / full local control, use the **OSS CLI**.
...
2. **ZeroDay installed** — check with `zeroday --version`. Install if missing:
   ```bash
   curl -sSL https://zeroday.ai/install | bash   # or: pipx install zeroday-agent
   ```
3. **LLM configured** — two environment variables:
   ```bash
   export ZERODAY_LLM="openai/gpt-5.4"      # any LiteLLM model id (openai/..., anthropic/..., openrouter/...)
...
zeroday -n -t ./ --scan-mode standard --max-budget 10

# Deployed app / API (black-box)
zeroday -n -t https://staging.example.com --max-budget 20

# Repo + deployed app together (best coverage)
zeroday -n -t https://github.com/org/app -t https://staging.example.com

# Focused testing with credentials or scope hints
zeroday -n -t https://app.example.com \
...
zeroday -n -t ./openapi.yaml -t https://api.staging.example.com

# Many targets from a file, one per line
zeroday -n --target-list ./targets.txt --max-budget 30

# Give the agents a file to work with (wordlist, spec, notes) without making it a target
zeroday -n -t https://staging.example.com --workspace-file ./wordlist.txt --max-budget 20
```
...
| `--resume RUN_NAME` | Resume a prior run from `zeroday_runs/`, with its agent history and targets. Cannot be combined with `-t`. |
...
The same `zeroday` binary drives the managed platform. Every command starts with `zeroday cloud`. Full details — asset registration, source uploads, reports, PR reviews, schedules, webhooks, and billing — are in the **managed-pentesting-with-zeroday** skill. Minimal flow:
...
#   zeroday cloud login --scopes scans:read scans:write assets:read assets:write \
...
zeroday cloud domains add --domain staging.example.com --asset-type web_app
zeroday cloud domains verify <domain-id>

# 3. Launch and wait
zeroday cloud scans start --engagement-type live_test --domain-ids <domain-id> --wait

# 4. Read validated findings
zeroday cloud vulns list --severity critical
```

For a local repository, `zeroday cloud scans start --source .` uploads the working tree (needs `uploads:write`) and infers a code review. When credits run out, `zeroday cloud billing topup` starts an agent-payable Stripe challenge — the managed skill covers the payment flow. Output is JSON when stdout is not a terminal, so the commands compose in scripts.

The raw REST API works too (`https://app.zeroday.ai/api/v1`, org-scoped bearer token — see [docs.app.zeroday.ai](https://docs.app.zeroday.ai)). If Docker or local prerequisites are not already satisfied, use this path instead of trying to install infra.

---

## Reporting & next steps

Summarize findings by severity (critical/high/medium/low/info) and include the PoC evidence. To remediate and verify fixes (via either path), use the **fix-security-vulnerabilities-with-zeroday** skill. To wire scanning into CI/CD, use the **ci-security-scanning-with-zeroday** skill.
