...
from zeroday.agents.factory import build_zeroday_agent, make_child_factory
from zeroday.agents.prompt import render_system_prompt
from zeroday.config import load_settings
...
from zeroday.core.agents import AgentCoordinator
from zeroday.core.execution import (
    respawn_subagents,
    run_agent_loop,
)
from zeroday.core.execution import (
...
from zeroday.core.hooks import BudgetExceededError, ReportUsageHooks, recomputed_budget_flags
from zeroday.core.inputs import (
    build_root_task,
    build_scan_targets,
    build_scope_context,
    make_model_settings,
...
from zeroday.core.sessions import open_agent_session
from zeroday.report.state import get_global_report_state
from zeroday.runtime import session_manager
from zeroday.telemetry import set_scan_phase
from zeroday.telemetry.logging import set_scan_id, setup_scan_logging
from zeroday.tools.output_store import (
    WORKSPACE_SPILL_DIR,
...
if TYPE_CHECKING:
    ...
    from zeroday.runtime.status import StatusSink
    from zeroday.tools.mcp import (
        ConnectedMcpServer,
...
async def run_zeroday_scan(
    *,
    ...
    """Run or resume one ZeroDay scan against a sandbox.
    ...
    :func:`zeroday.runtime.session_manager.create_or_reuse`.
    ...
    command-line default) it reads ``~/.zeroday/mcp-servers.json`` itself. Either
    ...
    logger.info(
        "%s ZeroDay scan %s (image=%s, max_turns=%d, interactive=%s, run_dir=%s)",
    ...
    if not resolved_model:
        raise RuntimeError(
            "No LLM model configured. Set ZERODAY_LLM env or pass model= to run_zeroday_scan().",
        )
    ...
    from zeroday.tools.coverage.tools import hydrate_coverage_from_disk
    from zeroday.tools.notes.tools import hydrate_notes_from_disk
    from zeroday.tools.threat_model.tools import hydrate_threat_models_from_disk
    from zeroday.tools.todo.tools import hydrate_todos_from_disk
    ...
    try:
        ...
        run_config = RunConfig(
            ...
            model_provider=ZeroDayProvider(),
        ...
        # ~/.zeroday/mcp-servers.json here. Either way one shared engine routine
        ...
        root_agent = build_zeroday_agent(
            name="Root Agent",
            skills=skills,
            is_root=True,
            scan_mode=scan_mode,
        ...
        logger.warning(
            ...
            "Resume with 'zeroday --resume %s' once the limit clears.",
            scan_id,
            exc,
            scan_id,
        )
        ...
        logger.exception("ZeroDay scan %s failed", scan_id)
        if root_id is not None:
            with contextlib.suppress(Exception):
        ...
        logger.info("ZeroDay scan %s done", scan_id)
        teardown_logging()
