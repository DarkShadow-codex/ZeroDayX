"""Sandbox backend registry — selected via ZERODAY_RUNTIME_BACKEND (default: docker)."""
...
async def _docker_backend(
    *,
    ...
    Uses :class:`ZeroDayDockerSandboxClient` to inject NET_ADMIN /
    ...
    ZeroDay manages session lifetime explicitly via ``client.delete()`` so we
    trigger ``start()`` ourselves.
    """
    import docker
    from agents.sandbox.sandboxes.docker import DockerSandboxClientOptions

    from zeroday.runtime.docker_client import ZeroDayDockerSandboxClient

    client = ZeroDayDockerSandboxClient(docker.from_env())
    client.zeroday_bind_mounts = bind_mounts or []
...
def get_backend(name: str) -> SandboxBackend:
    ...
    if backend is None:
        ...
        raise ValueError(
            f"Unknown ZERODAY_RUNTIME_BACKEND: {name!r} (supported: {supported})",
        )
