...
from zeroday.runtime.backends import backend_supports_bind_mounts, get_backend
from zeroday.runtime.caido_bootstrap import bootstrap_caido
from zeroday.runtime.caido_handle import CaidoBootstrapHandle
...
def _host_identity_env() -> dict[str, str]:
    ...
    if callable(getuid) and callable(getgid):
        return {"ZERODAY_HOST_UID": str(getuid()), "ZERODAY_HOST_GID": str(getgid())}
    return {}
...
def extra_file_staging_dir(scan_id: str) -> Path:
    ...
    return Path(tempfile.mkdtemp(prefix=f"zeroday-extra-files-{safe}-"))
