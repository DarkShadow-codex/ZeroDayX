...
- **Implementation:** `agents.sandbox.capabilities.tools.view_image.ViewImageTool`
  (upstream `agents` SDK)
- **Wired in:** `zeroday/agents/factory.py` — added per-run via the SDK
  `Filesystem` capability.
- **ZeroDay defaults:** screenshots default to
...
- **Skill:** screenshot workflow lives in `zeroday/skills/tooling/agent_browser.md`.
- **Recovery:** vision-not-supported model rejections are auto-recovered
  via `zeroday.core.sessions.strip_all_images_from_session`, invoked from
  `zeroday/core/execution.py`.
