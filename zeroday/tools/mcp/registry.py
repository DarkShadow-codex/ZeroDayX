...
One :class:`McpRegistry` is built per run in :mod:`zeroday.core.runner`, stored in
...
zeroday-pro imports :class:`McpRegistry` to add its cloud connections into the
same registry and to attach a per-connection ``result_transform`` (its
sanitizer), which :func:`zeroday.tools.mcp.client.dispatch_mcp_call` applies at the
...
from zeroday.tools.mcp.session import SupervisedMcpSession


if TYPE_CHECKING:
    from agents.mcp import MCPServer

    from zeroday.tools.mcp.client import ResultTransform
    from zeroday.tools.mcp.config import McpConnectionConfig
...
# zeroday-pro all agree on one name.
...
# (not in the interface layer) so the engine, the OSS viewer, and zeroday-pro's
...
class McpConnectionEntry:
    ...
    ``session`` is the :class:`~zeroday.tools.mcp.session.SupervisedMcpSession` that
    ...
    (zeroday-pro's sanitizer uses it). ``provider`` is an optional source label
...
class McpConnectionRequest:
    ...
    at the single dispatch point (zeroday-pro's sanitizer; empty for the
...
class McpRegistry:
    ...
    Public API (zeroday-pro builds against it): the constructor, :meth:`add`,
    ...
    def add(
        ...
        *,
        ...
        owns (zeroday-pro's cloud sessions): it is adopted into a session that runs
...
def resolve_mcp_call(
    ...
    The single resolver both the OSS viewer and zeroday-pro's tracer read a
