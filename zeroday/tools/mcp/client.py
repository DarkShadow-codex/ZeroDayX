...
per-run :class:`~zeroday.tools.mcp.registry.McpRegistry`, and the agent reaches
...
from zeroday.tools.mcp.failures import HttpStatusRecorder
...
if TYPE_CHECKING:
    ...
    from zeroday.tools.mcp.registry import McpConnectionRequest, McpRegistry
...
class ConnectedMcpServer(NamedTuple):
    ...
    :class:`~zeroday.tools.mcp.registry.McpRegistry`; ``name`` and ``tool_count``
...
async def dispatch_mcp_call(
    ...
    *,
    ...
    - with a ``result_transform`` (zeroday-pro's sanitizer), hands the parsed
...
async def connect_mcp_servers(
    ...
    Each connection becomes a :class:`~zeroday.tools.mcp.session.SupervisedMcpSession`
    ...
    :class:`~zeroday.tools.mcp.registry.McpRegistry` from these sessions, and the
