...
(see :func:`zeroday.core.execution.spawn_child_agent`): give each connection its
...
Security: the connection's :class:`~zeroday.tools.mcp.config.McpConnectionConfig`
...
from zeroday.tools.mcp.config import DEFAULT_MAX_CONCURRENT_CALLS
...
class SupervisedMcpSession:
    ...
    - :meth:`adopt` for an *adopted* session: the caller already holds a connected
      server (zeroday-pro's cloud sessions, and the test fakes). There is no
    ...
    async def dispatch(
        ...
        *,
        ...
        """
        from zeroday.tools.mcp.client import dispatch_mcp_call
        ...
        if outcome.call_failure is not None:
            from zeroday.tools.mcp.client import _errored_tool_output
    ...
    async def _open(self) -> MCPServer:
        ...
        """
        from zeroday.tools.mcp.client import _build_server
