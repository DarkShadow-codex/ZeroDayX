"""Read the open-source user's MCP servers from ``~/.zeroday/mcp-servers.json``.

An open-source user lists the MCP servers they want the agent to reach in a
small JSON file. ZeroDay reads it at the start of a run and connects to each
...
_DEFAULT_PATH: Path = Path.home() / ".zeroday" / "mcp-servers.json"
_PATH_ENV_VAR = "ZERODAY_MCP_CONFIG"
# Per-run selection, set by the --mcp-server / --mcp-exclude CLI flags. Each is a
# comma-separated list of connection names.
_ONLY_ENV_VAR = "ZERODAY_MCP_ONLY"
_EXCLUDE_ENV_VAR = "ZERODAY_MCP_EXCLUDE"
...
def _apply_run_selection(configs: list[McpConnectionConfig]) -> list[McpConnectionConfig]:
    ...
    ``ZERODAY_MCP_ONLY`` (if set) keeps only the named connections; then
    ``ZERODAY_MCP_EXCLUDE`` drops any named connection. With neither set, every
    connection is kept.
    """
...
def load_user_mcp_configs(path: Path | None = None) -> list[McpConnectionConfig]:
    ...
    The path is ``path`` if given, else ``$ZERODAY_MCP_CONFIG``, else
    ``~/.zeroday/mcp-servers.json``. The file is a JSON list of server entries.
