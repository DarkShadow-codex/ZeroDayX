"""`zeroday auth` — ChatGPT subscription sign-in (login / status / logout).

Signing in only stores OAuth tokens (``~/.zeroday/subscription-auth.json``); model
selection stays with ``ZERODAY_LLM``. A ``chatgpt/<model>`` ZERODAY_LLM runs on the
subscription.
"""
...
from zeroday.config import codex, load_settings
...
def run_auth(argv: list[str]) -> int:
    """Entry point for ``zeroday auth …``. Returns a process exit code."""
    console = Console()
    # Bare `zeroday auth` (no subcommand) defaults to login.
...
def _login(console: Console, argv: list[str]) -> int:
    parser = argparse.ArgumentParser(prog="zeroday auth login", add_help=True)
    parser.add_argument(
        "provider",
        nargs="?",
...
def _status(console: Console) -> int:
    ...
    if record is None:
        console.print(
            "[yellow]Not signed in.[/] Run [cyan]zeroday auth login chatgpt[/] to sign in."
        )
        return 1
    ...
    if codex.subscription_model(settings.llm.model):
        console.print(f"  Runs use the subscription (ZERODAY_LLM=[bold]{settings.llm.model}[/]).")
    else:
        console.print(
            "  [yellow]Note:[/] set [cyan]ZERODAY_LLM[/] to e.g. [cyan]chatgpt/gpt-5.4[/] "
...
def _print_success(console: Console) -> None:
    ...
    text.append("ZERODAY_LLM", style="bold white")
    ...
    text.append("zeroday --target https://example.com", style="bold cyan")
    console.print()
    console.print(
        Panel(
            text,
...
def _logo_img_tag() -> str:
    """Return an ``<img>`` for the ZeroDay logo as an inline data URI, or "".
...
<body>
  <a class="topbar" href="https://zeroday.ai" target="_blank" rel="noopener"
     aria-label="ZeroDay — zeroday.ai">
    <!--LOGO-->
    <span>ZeroDay</span>
  </a>
  <div class="brand">ZeroDay</div>
  <h1>You're signed in</h1>
  <main class="card">
    <div class="badge">✓</div>
    <p class="msg">ZeroDay is connected to your ChatGPT subscription. Head back to your
      terminal — your security test runs there.</p>
    <div class="rule"></div>
    <p class="tagline">Autonomous AI hackers that <b>find and fix</b> your app's
      vulnerabilities.</p>
    <nav class="links">
      <a href="https://zeroday.ai" target="_blank" rel="noopener">zeroday.ai</a>
      <span class="dot">·</span>
      <a href="https://docs.zeroday.ai" target="_blank" rel="noopener">docs</a>
      <span class="dot">·</span>
      <a href="https://discord.gg/zeroday-ai" target="_blank" rel="noopener">community</a>
    </nav>
  </main>
