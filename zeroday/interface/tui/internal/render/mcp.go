...
//
// Its own icon and color so a call that left ZeroDay for a server the user
...
// result is in the event data, the run log, and the `zeroday view` viewer.
...
// showing: ZeroDay assembled them itself from the run's registered connections,
