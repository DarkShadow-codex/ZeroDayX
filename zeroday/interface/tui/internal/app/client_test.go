...
func TestConnectFromEnvironmentAuthenticatesTCPTransport(t *testing.T) {
	...
	t.Setenv("ZERODAY_TUI_ADDR", listener.Addr().String())
	t.Setenv("ZERODAY_TUI_TOKEN", "one-use-token")
	t.Setenv("ZERODAY_TUI_FD", "")
	...
	if os.Getenv("ZERODAY_TUI_ADDR") != "" || os.Getenv("ZERODAY_TUI_TOKEN") != "" {
...
func TestConnectFromEnvironmentRequiresCompleteTransport(t *testing.T) {
	t.Setenv("ZERODAY_TUI_FD", "")
	t.Setenv("ZERODAY_TUI_ADDR", "127.0.0.1:1")
	t.Setenv("ZERODAY_TUI_TOKEN", "")

	_, err := ConnectFromEnvironment()
	if err == nil || !strings.Contains(err.Error(), "ZERODAY_TUI_ADDR and ZERODAY_TUI_TOKEN") {
		t.Fatalf("error = %v, want missing transport error", err)
	}
}

func TestConnectFromEnvironmentPrefersInheritedDescriptor(t *testing.T) {
	t.Setenv("ZERODAY_TUI_FD", "not-a-number")
	t.Setenv("ZERODAY_TUI_ADDR", "127.0.0.1:1")
	t.Setenv("ZERODAY_TUI_TOKEN", "token")

	_, err := ConnectFromEnvironment()
	if err == nil || !strings.Contains(err.Error(), "invalid ZERODAY_TUI_FD") {
