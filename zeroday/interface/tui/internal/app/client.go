...
func ConnectInherited(fdValue string) (*Client, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("invalid ZERODAY_TUI_FD: %w", err)
	}
	file := os.NewFile(uintptr(fd), "zeroday-tui-ipc")
	if file == nil {
		return nil, fmt.Errorf("invalid ZERODAY_TUI_FD %d", fd)
	}
...
func ConnectFromEnvironment() (*Client, error) {
	if fd := os.Getenv("ZERODAY_TUI_FD"); fd != "" {
		_ = os.Unsetenv("ZERODAY_TUI_FD")
		return ConnectInherited(fd)
	}

	address := os.Getenv("ZERODAY_TUI_ADDR")
	token := os.Getenv("ZERODAY_TUI_TOKEN")
	_ = os.Unsetenv("ZERODAY_TUI_ADDR")
	_ = os.Unsetenv("ZERODAY_TUI_TOKEN")
	if address == "" || token == "" {
		return nil, fmt.Errorf("ZERODAY_TUI_FD or ZERODAY_TUI_ADDR and ZERODAY_TUI_TOKEN are required")
	}
