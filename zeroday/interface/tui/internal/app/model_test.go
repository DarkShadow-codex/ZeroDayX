...
import (
	...
	"github.com/usezeroday/zeroday/tui/internal/protocol"
)
...
func TestAgentCollectionDeltaClearsErrorMessage(t *testing.T) {
	model := New(nil)
	failed := protocol.Agent{ID: "root", Name: "ZeroDay", Status: "failed", ErrorMessage: "provider rejected"}
	model.handleEnvelope(bootstrapEnvelope(t, "agents", 1, failed))

	resumed := protocol.Agent{ID: "root", Name: "ZeroDay", Status: "waiting"}
	delta := protocol.CollectionDelta{
...
func TestSetupStartScreenFitsNarrowTerminal(t *testing.T) {
	...
	if !strings.Contains(view, topRow) && !strings.Contains(view, "ZERODAY") {
...
func TestModalKeepsBackgroundVisible(t *testing.T) {
	...
	if !strings.Contains(view, "ZeroDay Help") {
...
func TestBudgetPauseShowsOneWarningToastUntilResumed(t *testing.T) {
	model := New(nil)
	model.snapshot.Agents = []protocol.Agent{{ID: "root", Name: "ZeroDay", Status: "budget_paused"}}
...
func TestLongErrorDoesNotBreakTheFrame(t *testing.T) {
	...
	bootstrap := protocol.CollectionBootstrap{
		...
		Items: []json.RawMessage{rawJSON(t, protocol.Agent{ID: "a0", Name: "ZeroDay", Status: "running"})},
	}
	model.handleEnvelope(protocol.Envelope{
	...
	if !strings.Contains(ansi.Strip(model.View()), "ZeroDay") {
