...
func (m Model) modalView() string {
	switch m.modal {
	case modalHelp:
		title := lipgloss.NewStyle().Bold(true).Foreground(green).Width(34).Align(lipgloss.Center).Render("ZeroDay Help")
	...
	case modalQuit:
		...
		return m.confirmView("Quit ZeroDay?", 24, dark, textColor)
	case modalStop:
		name := "agent"
