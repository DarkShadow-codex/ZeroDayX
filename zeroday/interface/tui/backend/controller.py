"""UI-independent state and command controller for interactive ZeroDay clients."""
...
from zeroday.config.models import is_recommended_or_frontier_model
from zeroday.config.settings import DEFAULT_MAX_TURNS
from zeroday.interface.tui.backend.live_view import TuiLiveView
from zeroday.interface.tui.backend.projection import (
    MAX_TERMINAL_EVENTS,
...
from zeroday.interface.utils import is_subscription_run


if TYPE_CHECKING:
    import argparse

    from zeroday.report.state import ReportState
...
class TuiController:
    ...
    async def _start(self, payload: dict[str, Any]) -> dict[str, Any]:
        ...
        model = (load_settings().llm.model or "").strip()
        if not model:
            raise ValueError("No model configured. Set ZERODAY_LLM first.")
    ...
    async def _open_viewer(self, _payload: dict[str, Any]) -> dict[str, Any]:
        ...
        try:
            from zeroday.interface.tui.backend.messages import (
                send_user_message_to_agent,
            )
            from zeroday.interface.viewer.server import (
                authorized_url,
                bundle_is_built,
                serve,
            )
