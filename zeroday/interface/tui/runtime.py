...
from zeroday.config import load_settings, persist_current
from zeroday.core.agents import AgentCoordinator
from zeroday.core.hooks import BudgetExceededError
from zeroday.core.runner import run_zeroday_scan
...
from zeroday.interface.tui.backend import TuiBackendServer, TuiController
from zeroday.interface.tui.backend.live_view import TuiLiveView
from zeroday.interface.tui.sidecar import (
    check_return_code,
    child_environment,
    launch_tui_process,
    package_version,
...
from zeroday.interface.utils import read_workspace_files
from zeroday.report.state import ReportState, set_global_report_state
from zeroday.telemetry import report_error, set_scan_phase
from zeroday.utils.resource_paths import get_zeroday_resource_path
...
class GoTuiRuntime:
    ...
    async def _run_scan(self) -> None:
        image = str(load_settings().runtime.image or "zeroday-sandbox:latest")
        try:
            await run_zeroday_scan(
    ...
    def binary_command() -> list[str]:
        ...
        if (source / "go.mod").is_file() and shutil.which("go"):
            return ["go", "run", "./cmd/zeroday-tui"]
        packaged = get_zeroday_resource_path("bin", tui_executable())
        if packaged.is_file():
            return [str(packaged)]
        raise RuntimeError(
            "Bubble Tea TUI binary not found. Reinstall ZeroDay from an official platform wheel."
        )
    ...
    async def run(self) -> None:
        ...
        try:
            ...
            env["ZERODAY_VERSION"] = package_version()
