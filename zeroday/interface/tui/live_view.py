...
from zeroday.core.paths import runtime_state_dir
from zeroday.interface.tui.history import load_session_history
from zeroday.tools.mcp import resolve_mcp_call
...
class TuiLiveView:
    ...
    def _mcp_tool_fields(self, tool_name: str, args: dict[str, Any]) -> dict[str, str]:
        ...
        dispatch call is attributed the same way here and in zeroday-pro's tracer.
...
_INTERNAL_TURN_PREFIXES = (
    # zeroday.core.agents._message_to_session_item: everything the coordinator
    # delivers from another agent or from the system, which wraps the stall,
    # terminal and budget-extension notices in zeroday.core.execution too.
    "[Message from ",
    # zeroday.core.inputs.child_initial_input: a subagent's parent context.
    "== Inherited context from parent",
    # zeroday.core.execution: the no-tool-call recovery nudge, both modes.
    ...
    # zeroday.core.hooks: budget warnings, the only notices injected unwrapped.
    *(
        f"[{label}] {subject}"
