...
def tui_executable() -> str:
    return "zeroday-tui.exe" if os.name == "nt" else "zeroday-tui"
...
async def _launch_posix_tui_process(
    ...
    try:
        env["ZERODAY_TUI_FD"] = str(child_socket.fileno())
...
async def _launch_windows_tui_process(
    ...
    try:
        ...
        env.update({"ZERODAY_TUI_ADDR": f"{host}:{port}", "ZERODAY_TUI_TOKEN": token})
