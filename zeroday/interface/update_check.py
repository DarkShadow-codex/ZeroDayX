"""Update notifications and self-update for the zeroday CLI.
...
result in ``~/.zeroday``, a non-intrusive notice with the upgrade command
for the detected install method, and a ``zeroday --update`` self-update
...
from zeroday.telemetry._common import get_version


logger = logging.getLogger(__name__)

GITHUB_REPO = "usezeroday/zeroday"
PYPI_PACKAGE = "zeroday-agent"
CHECK_INTERVAL_SECONDS = 24 * 60 * 60
REQUEST_TIMEOUT_SECONDS = 5

_CACHE_PATH = Path.home() / ".zeroday" / "update-check.json"

_background_thread: threading.Thread | None = None


def _is_disabled() -> bool:
    return bool(os.environ.get("ZERODAY_NO_UPDATE_CHECK")) or any(
        os.environ.get(key)
...
def get_upgrade_command(method: str | None = None) -> str:
    ...
    commands = {
        "binary": "zeroday --update",
        "pipx": "pipx upgrade zeroday-agent",
        "uv": "uv tool upgrade zeroday-agent",
        "pip": "pip install --upgrade zeroday-agent",
    }
...
def notify_update(console: Console) -> None:
    ...
    if not latest:
        return
    console.print(
        f"[#eab308]A new version of zeroday is available:[/] "
...
def run_package_upgrade(console: Console, method: str) -> bool:
    ...
    if result.returncode != 0:
        ...
        return False
    console.print("[#22c55e]✓ zeroday updated — restart the scan to use the new version[/]")
    return True
...
def prompt_update_if_available(console: Console) -> bool:
    ...
    Returns True if zeroday was updated (caller should re-exec / exit).
    """
    latest = get_available_update()
    ...
    choice = Prompt.ask("Update zeroday?", choices=["y", "n", "s"], default="n")
    console.print()
    if choice == "s":
        skip_version(latest)
        return False
...
def _download_and_replace(version: str, target: str, console: Console) -> bool:
    ...
    filename = f"zeroday-{version}-{target}{archive_ext}"
    url = f"https://github.com/{GITHUB_REPO}/releases/download/v{version}/{filename}"
    binary_name = f"zeroday-{version}-{target}" + (".exe" if is_windows else "")
...
def self_update(console: Console | None = None, version: str | None = None) -> bool:
    ...
    if not is_binary_install():
        method = get_install_method()
        console.print(
            f"[#eab308]This zeroday was installed via {method};[/] "
    ...
    if not latest:
        console.print("[bold red]Could not determine the latest zeroday version.[/]")
        return False

    current = get_version()
    if current != "unknown" and not _is_newer(latest, current):
        console.print(f"[#22c55e]zeroday {current} is already the latest version.[/]")
        return True
    ...
    try:
        ...
        console.print(
            ...
            "[#60a5fa]curl -sSL https://zeroday.ai/install | bash[/]"
        )
        return False

    _write_cache(latest_version=latest, checked_at=time.time())
    console.print(f"[#22c55e]✓ Updated zeroday to {latest}[/]")
    return True
