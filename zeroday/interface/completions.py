"""Shell completion scripts and candidates for the ZeroDay CLI."""
...
from zeroday.interface.cloud.spec import DEFAULT_VERBS, SPEC, Cmd
from zeroday.interface.terminal_text import has_terminal_control, sanitize_terminal_text
...
def run_completions(argv: list[str]) -> int:
    ...
    if not argv or argv[0] in ("-h", "--help", "help"):
        sys.stdout.write(
            "Usage: zeroday completions <zsh|bash|fish>\n\n"
            "Enable tab completion for the current shell:\n"
            "  zsh:  source <(zeroday completions zsh)\n"
            "  bash: source <(zeroday completions bash)\n"
            "  fish: zeroday completions fish | source\n"
        )
        return 0
    shell = argv[0].lower()
...
def completion_candidates(words: list[str]) -> list[str]:
    """Return candidates for words after the ``zeroday`` executable."""
...
def _zsh_script() -> str:
    return r"""#compdef zeroday
_zeroday() {
  local -a candidates
  candidates=("${(@f)$($words[1] completions --candidates "${words[@]:2}")}")
  _describe 'zeroday' candidates
}
compdef _zeroday zeroday
"""


def _bash_script() -> str:
    return r"""_zeroday_completion() {
  local -a candidates
  local candidate
  while IFS= read -r candidate; do
    candidates+=("$candidate")
  done < <(zeroday completions --candidates "${COMP_WORDS[@]:1:$COMP_CWORD}")
...
complete -F _zeroday_completion zeroday
"""


def _fish_script() -> str:
    return r"""function __zeroday_candidates
  set -l words (commandline -opc)
  set -e words[1]
  command zeroday completions --candidates $words (commandline -ct)
end
complete -c zeroday -f -a '(__zeroday_candidates)'
"""
