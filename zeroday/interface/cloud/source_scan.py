...
from zeroday.interface.cloud.source_upload import prepare_source, remove_bundle
...
if TYPE_CHECKING:
    ...
    from zeroday.interface.cloud.source_upload import SourceBundle
...
def prepare_scan_source(
    ...
    try:
        ...
        console.print(
            ...
            "  Only the selected files will be sent to ZeroDay Cloud."
        )
...
def _upload_scan_source(bundle: SourceBundle, *, token: str | None) -> str:
    file_name = f"zeroday-source-{bundle.archive_sha256[:12]}.zip"
...
def _source_cleanup_note(upload_id: str, cleanup_error: BaseException) -> str:
    return (
        ...
        f"Retry with `zeroday cloud uploads delete {upload_id}`."
    )
...
def _interrupted_source_upload_error(
    ...
    message = (
        ...
        f"{upload_id} was retained. Check `zeroday cloud scans list` before retrying; if no scan "
        f"was created, run `zeroday cloud uploads delete {upload_id}`.{retry_note}"
    )
    payload: dict[str, Any] = {
        "error": message,
        "interrupted": True,
...
def _retained_source_upload_error(
    ...
    message = (
        ...
        "Check `zeroday cloud scans list` before retrying; if no scan was created, clean it up "
        f"with `zeroday cloud uploads delete {upload_id}`. Linked uploads cannot be deleted."
        f"{retry_note}"
    )
    payload: dict[str, Any] = {}
    exit_code = http.EXIT_ERROR
