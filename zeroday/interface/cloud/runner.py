"""Generic command runner for `zeroday cloud`.
...
from zeroday.interface.cloud.arguments import CloudArgumentParser
from zeroday.interface.cloud.billing import run_topup
from zeroday.interface.cloud.render import emit, json_mode
from zeroday.interface.cloud.source_scan import LocalSourceScan
from zeroday.interface.cloud.spec import DEFAULT_VERBS, SPEC, Cmd, P
...
def _execute(  # noqa: PLR0912, PLR0915
    ...
    *,
    ...
    emit(
        ...
        hint=(
            "Switch with `zeroday cloud workspaces use NUMBER`."
            if workspace_list
            else (
                "For Git providers, disconnect with `zeroday cloud integrations disconnect "
...
def _build_parser(group: str, verb_label: str, cmd: Cmd) -> argparse.ArgumentParser:
    parser = CloudArgumentParser(prog=f"zeroday cloud {group} {verb_label}", description=cmd.help)
    for name in _PLACEHOLDER.findall(cmd.path):
    ...
    parser.add_argument(
        ...
        default=None,
        metavar="ORG_ID",
        help="Expected workspace for an override CLI token (or ZERODAY_WORKSPACE_ID).",
    )
...
def _interrupted_scan_launch_error(idempotency_key: str | None = None) -> http.CloudError:
    ...
    message = (
        ...
        f"`zeroday cloud scans list` before retrying.{retry_note}"
    )
    payload: dict[str, Any] = {
        "error": message,
        "interrupted": True,
...
def _ambiguous_scan_launch_error(
    ...
    retry_note = _idempotency_retry_note(idempotency_key)
    message = (
        f"{error} The scan launch outcome is unknown; check `zeroday cloud scans list` before "
