...
ZeroDay API token that way exposes it to process-listing tools.  This module keeps
the token in the ZeroDay process and injects it while forwarding the wallet's few
...
def _make_handler(state: _BridgeState) -> type[BaseHTTPRequestHandler]:
    class WalletBridgeHandler(BaseHTTPRequestHandler):
        ...
        def do_POST(self) -> None:  # noqa: PLR0911, PLR0912
            ...
            headers["X-ZeroDay-Authorization"] = state.authorization
            if state.workspace_id:
                headers["X-ZeroDay-Workspace"] = state.workspace_id
            try:
                ...
                _send_json_error(self, 502, "ZeroDay billing response was too large")
                return
            except requests.RequestException:
                _send_json_error(self, 502, "Could not reach the ZeroDay billing endpoint")
                return
            ...
            if 300 <= response_status < 400:
                _send_json_error(self, 502, "ZeroDay billing refused an unexpected redirect")
                return
...
def wallet_payment_bridge(
    *,
    ...
    """Yield a one-run loopback URL that injects the ZeroDay API token upstream.
