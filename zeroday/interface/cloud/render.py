"""Output rendering for `zeroday cloud` commands."""
    ...
    if view == "GET /scans":
        ...
        if scan_rows is not None:
            _print_table(
                ...
                row_numbers=False,
                omit_columns=omit_columns,
                hint=_combine_hints("Inspect one scan with `zeroday cloud scans get ID`.", hint),
                view=view,
            )
            return
    if view == "GET /vulnerabilities":
        ...
        if vulnerability_rows is not None:
            _print_table(
                ...
                hint=_combine_hints("Inspect one finding with `zeroday cloud vulns get ID`.", hint),
                view=view,
            )
            return
    if view == "GET /pr-reviews":
        ...
        if review_rows is not None:
            _print_table(
                ...
                omit_columns=omit_columns,
                hint=_combine_hints(
                    "Use `zeroday cloud pr-reviews get ID` for one review.",
...
def _view_hint(data: Any, view: str | None) -> str | None:
    ...
    if view == "GET /scans/{scanId}/trace" and _is_record(data):
        ...
        hint = "Inspect one schedule with `zeroday cloud schedules get ID`."
    return hint
...
def _trace_view_hint(data: dict[str, Any]) -> str | None:
    ...
    parts = [
        f"Inspect a complete event with `zeroday cloud scans trace-event{suffix} EVENT_ID`; "
...
def _print_chat_credentials(console: Console, data: dict[str, Any]) -> None:
    ...
    sections = (
        ...
        (
            ...
            "chat_credentials_test_users",
            "Attach one with `zeroday cloud chat credentials set CHAT_ID --test-user-ids ID`.",
        ),
        (
            ...
            "chat_credentials_scans",
            (
                "Discover these with `zeroday cloud chat credentials CHAT_ID --scan-ids SCAN_ID`; "
                "attach them with `zeroday cloud chat credentials set CHAT_ID --scan-ids SCAN_ID`."
            ),
        ),
    )
...
def _print_supply_chain_summary(console: Console, data: dict[str, Any]) -> None:
    ...
    _print_table(
        ...
        hint="Inspect one repository with `zeroday cloud repos supply-chain summary ID`.",
        show_json_hint=False,
    )
