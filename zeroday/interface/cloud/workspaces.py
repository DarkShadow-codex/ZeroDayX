"""`zeroday cloud workspaces use` — switch the stored token to another workspace.
...
from zeroday.interface.platform_cli import AUTH_PATH, read_record, save_record
...
def run_workspace_use(argv: list[str]) -> int:
    """Entry point for ``zeroday cloud workspaces use``. Returns an exit code."""
    console = Console()
    parser = CloudArgumentParser(
        prog="zeroday cloud workspaces use",
...
def _use(  # noqa: PLR0912, PLR0915
    ...
    external_token = args.token is not None or bool(os.environ.get("ZERODAY_API_TOKEN", "").strip())
...
def _find_workspace(selector: str, *, token: str | None) -> dict[str, Any]:
    ...
    if wanted.isdigit():
        ...
        raise http.CloudError(
            ...
            "Run `zeroday cloud workspaces` to see the numbered list."
        )
