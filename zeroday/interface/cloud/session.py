"""Inspect and safely narrow a managed ZeroDay CLI session."""
...
from zeroday.interface.platform_cli import read_record, save_record
...
def _configure(args: argparse.Namespace) -> bool:
    external = args.token is not None or bool(os.environ.get("ZERODAY_API_TOKEN", "").strip())
    http.configure(
...
def _run_show(console: Console, argv: list[str], *, scopes_only: bool) -> int:
    subcmd = "scopes" if scopes_only else "show"
    parser = CloudArgumentParser(prog=f"zeroday cloud session {subcmd}")
    _common(parser)
...
def _run_scopes_set(console: Console, argv: list[str]) -> int:
    parser = CloudArgumentParser(
        prog="zeroday cloud session scopes set",
...
def _print_help(console: Console) -> None:
    console.print("[bold]zeroday cloud session[/] commands:")
