"""HTTP client for the managed ZeroDay platform API (app.zeroday.ai)."""
...
from zeroday.interface.platform_cli import read_record
...
TOPUP_COMMAND = "zeroday cloud billing topup --credits <count>"
BALANCE_COMMAND = "zeroday cloud billing credits"
...
def configure(
    *,
    ...
    explicit_workspace = workspace_id or os.environ.get("ZERODAY_WORKSPACE_ID")
    if explicit_workspace:
        _workspace_id_override = explicit_workspace.strip()
    elif not token_override and not os.environ.get("ZERODAY_API_TOKEN"):
        record = read_record()
...
def app_url() -> str:
    ...
    explicitly_configured = bool(os.environ.get("ZERODAY_APP_URL")) or "app_url" in getattr(
        viewer, "model_fields_set", set[str]()
    )
    if explicitly_configured or _token_override_active or os.environ.get("ZERODAY_API_TOKEN"):
        return configured
    record = read_record()
...
def api_token(override: str | None = None) -> str:
    token = override or os.environ.get("ZERODAY_API_TOKEN")
    if not token:
        record = read_record()
    ...
    if not token or not token.strip():
        raise CloudError(
            "not signed in. Run `zeroday cloud login`, or set ZERODAY_API_TOKEN.",
            exit_code=EXIT_AUTH,
        )
    return token.strip()
...
def _validate_stored_token_origin(record: dict[str, Any]) -> None:
    ...
    if not isinstance(stored_url, str) or not stored_url:
        raise CloudError(
            "the stored sign-in is not bound to a trusted platform. Run `zeroday cloud login` "
    ...
    try:
        ...
        raise CloudError(
            "the stored sign-in has an invalid platform binding. Run `zeroday cloud login` again.",
            exit_code=EXIT_AUTH,
        ) from exc
    if stored_origin != active_origin:
        raise CloudError(
            "the stored sign-in belongs to a different platform. Refusing to send its token; "
            "run `zeroday cloud login` for the configured platform or supply an explicit token.",
            exit_code=EXIT_AUTH,
        )
...
def expected_workspace_id(*, token_override: bool) -> str | None:
    ...
    if token_override or _token_override_active or os.environ.get("ZERODAY_API_TOKEN"):
        return None
    return None
...
def check(response: requests.Response) -> Any:
    ...
    if 200 <= response.status_code < 300:
        ...
        if "application/json" not in content_type:
            raise CloudError(
                "the server returned a non-JSON response. Check ZERODAY_APP_URL and preview "
                "access, then retry."
            )
        try:
            return response.json()
            ...
            raise CloudError(
                "the server returned malformed JSON. Check ZERODAY_APP_URL and preview "
                "access, then retry."
            ) from exc
    detail = ""
    error_code = ""
