"""Billing top-up and agent-wallet execution for ``zeroday cloud``."""
...
import zeroday.interface.cloud.http as http  # noqa: PLR0402
from zeroday.interface.cloud.payment_proxy import WalletUpstreamResponse, wallet_payment_bridge
from zeroday.interface.cloud.render import emit
...
_LINK_CLI_CLIENT_NAME = "ZeroDay CLI"
    ...
    *,
        ...
        emit(
            console,
            {
                "error": (
                    ...
                    "run `zeroday cloud billing credits` and check the balance before retrying."
                ),
                "interrupted": True,
    ...
    if not as_json:
        console.print(
            ...
            "unknown; run `zeroday cloud billing credits` before retrying.[/]"
        )
    ...
    if result.returncode == 0:
        ...
        emit(
            ...
            {
                "error": (
                    "The wallet returned a receipt, but the ZeroDay billing endpoint did not "
...
def _run_wallet_client(
    ...
    *,
    ...
    with tempfile.TemporaryDirectory(prefix="zeroday-wallet-") as wallet_cwd:
...
def _wallet_npm_cache() -> Path:
    ...
    cache = Path.home() / ".zeroday" / "wallet-npm-cache"
...
def _payment_context(body: dict[str, Any]) -> str:
    ...
    return (
        f"ZeroDay scan credits. The ZeroDay command line interface asks to buy "
        f"{credits_requested} scan credit(s) for the selected ZeroDay workspace on "
        "app.zeroday.ai. ZeroDay spends the credits on managed penetration test scans "
...
def _prepare_link_wallet(console: Console, npx: str, *, as_json: bool) -> str | None:
    ...
    manual_setup = (
        "Payment needs a Stripe Link wallet. Run `zeroday cloud billing topup` in an "
        "interactive terminal to connect one, or set up the wallet at "
        "https://link.com/agents. For a browser checkout instead, run "
        "`zeroday cloud billing subscribe --plan zeroday_top_up`."
    )
    if as_json or not (sys.stdin.isatty() and sys.stdout.isatty()):
        return manual_setup

    console.print(
        "[yellow]No Stripe Link wallet is connected.[/] ZeroDay starts the Link sign-in now. "
        "Approve the connection in the Link app, then ZeroDay continues the payment. "
