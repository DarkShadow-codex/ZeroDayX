"""`zeroday cloud` — the managed ZeroDay platform (app.zeroday.ai) from the terminal.
...
from zeroday.interface.cloud import http
from zeroday.interface.cloud.render import json_mode
from zeroday.interface.cloud.runner import resolve, run
from zeroday.interface.cloud.session import run_session
from zeroday.interface.cloud.spec import DEFAULT_VERBS, GROUP_HELP, SPEC
from zeroday.interface.cloud.workspaces import run_workspace_use
from zeroday.interface.platform_cli import run_login
from zeroday.interface.terminal_text import sanitize_terminal_text


_USAGE_HEADER = """[bold]Usage:[/] zeroday cloud <command> [arguments]
...
_USAGE_FOOTER = """
Run [bold]zeroday cloud <command> help[/] to list its verbs. Common read-only
...
API reference: https://docs.app.zeroday.ai"""
...
def _run_cloud(argv: list[str]) -> int:  # noqa: PLR0911, PLR0912
    """Entry point for ``zeroday cloud …``. Returns a process exit code."""
    console = Console()
...
def _print_verbs(
    ...
    if as_json:
        ...
        return
    console.print(f"[bold]zeroday cloud {group}[/] verbs:")
