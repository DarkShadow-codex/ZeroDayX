"""`zeroday cloud login` — managed platform sign-in (app.zeroday.ai).

Signing in runs an OAuth 2.0 device authorization flow in the browser, creates
the ZeroDay account and workspace when they do not exist yet, and stores a
personal API token in ``~/.zeroday/platform-auth.json``. The token drives the
...
from zeroday.interface.platform_identity import read_or_create_identity
from zeroday.interface.terminal_text import sanitize_terminal_text
from zeroday.interface.url_safety import is_safe_web_url
from zeroday.utils.secret_files import write_secret_text


AUTH_PATH = Path.home() / ".zeroday" / "platform-auth.json"
...
def run_login(argv: list[str]) -> int:
    """Entry point for ``zeroday cloud login``. Returns a process exit code."""
    console = Console()
...
def _login(console: Console, argv: list[str]) -> int:
    parser = argparse.ArgumentParser(prog="zeroday cloud login", add_help=True)
    parser.add_argument(
        "--no-browser",
        action="store_true",
    ...
    console.print(f"[bold]Signing in to the ZeroDay platform[/] [dim]({_terminal_markup(host)})[/]")
    console.print(
    ...
    try:
        ...
        console.print(
            ...
            "then run `zeroday cloud login` again.[/]"
        )
        return 1
...
def _run_device_flow(  # noqa: PLR0912, PLR0915
    ...
    *,
    ...
    raise PlatformAuthError("the sign-in request expired. Run `zeroday cloud login` again.")
...
def _session_headers(record: dict[str, Any]) -> dict[str, str]:
    ...
    if isinstance(workspace_id, str) and workspace_id:
        headers["X-ZeroDay-Workspace"] = workspace_id
    return headers
...
def _print_success(console: Console, record: dict[str, Any]) -> None:
    ...
    console.print()
    console.print("[green]✓ Signed in to the ZeroDay platform.[/]")
    if email:
    ...
    console.print()
    console.print(
        "[dim]The managed platform is ready. Run `zeroday cloud` to list the commands. "
        "See https://docs.app.zeroday.ai for the API reference.[/]"
    )


def _status(console: Console, argv: list[str]) -> int:  # noqa: PLR0912
    parser = _SessionArgumentParser(
        prog="zeroday cloud whoami",
    ...
    if record is None:
        if as_json:
            ...
            return 1
        console.print("[yellow]Not signed in.[/] Run [bold]zeroday cloud login[/] to sign in.")
        return 1
