...
telemetry live here so ``zeroday.interface.main`` (the CLI) and
``zeroday.interface.tui.runtime`` (interactive setup) depend on one module
...
from zeroday.config import Settings, codex, load_settings
...
from zeroday.utils.api_spec import (
    SpecParseError,
...
async def preflight_model_connection(
    ...
    *,
    ...
    from zeroday.config.models import ZeroDayProvider, configure_sdk_model_defaults
    ...
    model = ZeroDayProvider().get_model(model_name)
...
def _persist_run_record(args: argparse.Namespace) -> None:
    from zeroday.report.writer import write_run_record
