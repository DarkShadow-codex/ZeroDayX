...
from zeroday.telemetry import report_error
from zeroday.utils.api_spec import detect_spec_format
...
def _resolve_repo_diff_scope(
    ...
    if _is_repo_shallow(repo_path):
        raise ValueError(
            "ZeroDay requires full git history for diff-scope. Please set fetch-depth: 0 "
            "in your CI config."
        )
...
def write_fetched_collection(collection: dict[str, Any], collection_uid: str) -> str:
    ...
    """
    staging = Path(tempfile.gettempdir()) / "zeroday_api_specs" / "fetched"
...
def stage_api_specs(targets_info: list[dict[str, Any]], run_name: str) -> list[dict[str, Any]]:
    ...
    staging = Path(tempfile.gettempdir()) / "zeroday_api_specs" / run_name
    staging.mkdir(parents=True, exist_ok=True)
...
def clone_repository(repo_url: str, run_name: str, dest_name: str | None = None) -> str:
    ...
    temp_dir = Path(tempfile.gettempdir()) / "zeroday_repos" / run_name
...
def _split_workspace_file_spec(spec: str) -> tuple[str, str | None]:
    """Split a ``PATH[:DEST]`` spec into (source, dest) without mistaking Windows drive letters."""
    stripped = spec.strip()
    if Path(stripped).expanduser().is_file():
        return stripped, None

    if len(stripped) >= 2 and stripped[0].isalpha() and stripped[1] == ":":
        sep_index = stripped.rfind(":")
        if sep_index > 1:
            raw = stripped[:sep_index]
            dest = stripped[sep_index + 1 :].strip()
            return raw, dest or None
        return stripped, None

    raw, sep, dest_part = stripped.rpartition(":")
    if sep and dest_part.strip():
        return raw, dest_part.strip()
    return stripped, None
