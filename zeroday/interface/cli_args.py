"""Command-line argument parsing for the ``zeroday`` scan entrypoint."""
...
from zeroday.config import apply_config_override
from zeroday.config.settings import DEFAULT_MAX_TURNS
from zeroday.core.paths import run_dir_for, runtime_state_dir
from zeroday.interface.scan_setup import attach_workspace_mount, build_targets_info
from zeroday.interface.update_check import self_update
from zeroday.interface.utils import (
    check_mountable_dir,
...
def parse_arguments() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="ZeroDay Multi-Agent Cybersecurity Penetration Testing Tool",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  # Web application penetration test
  zeroday --target https://example.com

  # GitHub repository analysis
  zeroday --target https://github.com/user/repo
  zeroday --target git@github.com:user/repo.git

  # Local code analysis
  zeroday --target ./my-project

  # API spec test (OpenAPI/Swagger file or Postman collection export)
  zeroday --target ./openapi.yaml --target https://api.example.com
  zeroday --target ./collection.postman_collection.json

  # Postman collection pulled live by id (needs POSTMAN_API_KEY); optional environment
  zeroday --target postman://<collection-uuid> --target https://api.example.com
  zeroday --target "postman://<collection-uuid>?env=<environment-uuid>"

  # Domain penetration test
  zeroday --target example.com

  # IP address penetration test
  zeroday --target 192.168.1.42

  # Multiple targets (e.g., white-box testing with source and deployed app)
  zeroday --target https://github.com/user/repo --target https://example.com
  zeroday --target ./my-project --target https://staging.example.com --target https://prod.example.com

  # Targets from a file, one target per non-empty, non-comment line
  zeroday --target-list ./targets.txt

  # Custom instructions (inline)
  zeroday --target example.com --instruction "Focus on authentication vulnerabilities"

  # Custom instructions (from file)
  zeroday --target example.com --instruction-file ./instructions.txt
  zeroday --target https://app.com --instruction-file /path/to/detailed_instructions.md

  # Extra files placed in the sandbox workspace
  zeroday --target ./my-project --workspace-file ./wordlist.txt
  zeroday --target https://app.com --workspace-file ./openapi.yaml:specs/openapi.yaml

ZeroDay Cloud:
  zeroday cloud login
  zeroday cloud scans start --source . --yes --wait
  zeroday cloud                      # list every cloud resource

  Run a pentest in ZeroDay Cloud     https://app.zeroday.ai
  Try ZeroDay Enterprise             https://zeroday.ai/demo
        """,
    )
    ...
    parser.add_argument(
        ...
        "--version",
        action="version",
        version=f"zeroday {get_version()}",
    )

    parser.add_argument(
        "--update",
        action="store_true",
        help="Update zeroday to the latest version and exit. Self-updates the "
    ...
    parser.add_argument(
        "--config",
        type=str,
        help="Path to a custom config file (JSON) to use instead of ~/.zeroday/cli-config.json",
    )
    ...
    parser.add_argument(
        ...
        type=str,
        metavar="PATH",
        help="Path to an MCP servers JSON file to use instead of ~/.zeroday/mcp-servers.json.",
    )
    ...
    parser.add_argument(
        ...
        metavar="RUN_NAME",
        help=(
            "Resume a prior scan by its run name (the dir under ./zeroday_runs/). "
    ...
    if args.mcp_config:
        ...
        os.environ["ZERODAY_MCP_CONFIG"] = str(mcp_config_path)

    # The MCP loader reads these as its per-run include/exclude selection.
    if args.mcp_server:
        os.environ["ZERODAY_MCP_ONLY"] = ",".join(args.mcp_server)
    if args.mcp_exclude:
        os.environ["ZERODAY_MCP_EXCLUDE"] = ",".join(args.mcp_exclude)
...
def _load_resume_state(args: argparse.Namespace, parser: argparse.ArgumentParser) -> None:
    ...
    from zeroday.report.writer import read_run_record
