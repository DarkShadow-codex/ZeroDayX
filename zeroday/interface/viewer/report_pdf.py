...
The layout mirrors the ZeroDay cloud pentest report (cover page, executive
...
the user's own hands. ZeroDay cannot read the delivered report.
"""
...
def _cover(
    ...
    header = Table(
        [[_LogoMark(30), Paragraph("ZeroDay", styles["wordmark"])]],
...
def build_encrypted_report(run_dir: Path) -> tuple[bytes, str, str]:
    ...
    filename = f"zeroday-report-{run_name}.pdf"
