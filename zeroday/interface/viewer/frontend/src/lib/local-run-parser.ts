...
/**
 * Pure, dependency-free parsers that turn a ZeroDay CLI local run
 * (`zeroday_runs/<run>/{run.json,vulnerabilities.json}`) into the app's own
...
function parseJson(text: string, label: string): unknown {
  try {
    ...
    throw new RunParseError(
      `${label} isn't valid JSON. Make sure you selected a ZeroDay run directory.`
    );
  }
}
