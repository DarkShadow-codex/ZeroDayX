...
// TUI strips them in zeroday/interface/tui/renderers/shell_renderer.py). Only a
...
function cleanOutput(raw: string, command: string = ""): string {
  ...
  if (cleaned.trim()) {
    ...
    for (const line of lines) {
      ...
      // Skip [ZERODAY_N]$ prompt lines
      if (/^\[ZERODAY_\d+\]\$\s*/.test(line)) continue;
    ...
    // Strip trailing [ZERODAY_N]$ lines
    while (filtered.length > 0 && /^\[ZERODAY_\d+\]\$\s*/.test(filtered[filtered.length - 1])) {
      filtered.pop();
    }
