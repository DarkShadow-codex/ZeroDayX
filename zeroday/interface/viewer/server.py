...
from zeroday.core.paths import run_record_path
from zeroday.interface.viewer import auth
...
# port is appended (``zeroday_viewer_session_<port>``) because browsers scope
...
SESSION_COOKIE_PREFIX = "zeroday_viewer_session"
...
class _ViewerState:
    def __init__(
        ...
        # The zeroday_runs directory that holds the launched run; used to
        ...
        # standalone ``zeroday view`` / finished runs, so steering is unavailable.
...
def _make_handler(state: _ViewerState) -> type[BaseHTTPRequestHandler]:
    class ViewerHandler(BaseHTTPRequestHandler):
        server_version = "ZeroDayViewer/1.0"
        ...
        def _handle_event(self) -> None:
            ...
            if event == "cta_clicked":
                from zeroday.telemetry import posthog
        ...
        def _handle_report_send(self) -> None:
            ...
            from zeroday.interface.viewer.report_pdf import build_encrypted_report
...
def serve(
    ...
    *,
    ...
    use, falls back to an ephemeral port. Reused by both the ``zeroday view``
    ...
    ``None`` (standalone ``zeroday view``), steering is reported unavailable.
    """
    ...
    thread = threading.Thread(target=httpd.serve_forever, name="zeroday-viewer", daemon=True)
    thread.start()
