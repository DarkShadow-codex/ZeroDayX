...
from zeroday.config import IntegrationSettings, codex, load_settings
...
def validate_environment() -> None:
    ...
    if codex.subscription_model(settings.llm.model):
        if not codex.is_authenticated():
            console.print(
                f"[red]ZERODAY_LLM={settings.llm.model} uses your ChatGPT subscription, "
                "but you're not signed in.[/] Run [cyan]zeroday auth login chatgpt[/] first."
            )
    ...
    if not settings.llm.model:
        missing_required_vars.append("ZERODAY_LLM")
    ...
    if missing_required_vars:
        ...
        for var in missing_required_vars:
            if var == "ZERODAY_LLM":
                ...
                error_text.append("ZERODAY_LLM", style="bold cyan")
                error_text.append(
        ...
        if missing_optional_vars:
            ...
            for var in missing_optional_vars:
                ...
                elif var == "ZERODAY_REASONING_EFFORT":
                    error_text.append("• ", style="white")
                    error_text.append("ZERODAY_REASONING_EFFORT", style="bold cyan")
                    error_text.append(
        ...
        error_text.append("\nExample setup:\n", style="white")
        error_text.append("export ZERODAY_LLM='openrouter/z-ai/glm-5.3'\n", style="dim white")
        ...
        if missing_optional_vars:
            for var in missing_optional_vars:
                if var == "LLM_API_BASE":
                    ...
                    error_text.append(
                        "export ZERODAY_REASONING_EFFORT='high'\n",
                        style="dim white",
                    )
