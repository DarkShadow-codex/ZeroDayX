...
from zeroday.config import codex
from zeroday.config.loader import load_settings
from zeroday.config.tool_call_ids import TurnCallIdRewriter, dedupe_input
from zeroday.config.tool_call_limits import TurnToolCallLimiter
...
if TYPE_CHECKING:
    ...
    from zeroday.config.settings import LlmSettings, ReasoningEffort, Settings
...
class _NonStreamingModel(Model):
    ...
    ZeroDay uses only issues streamed requests, so such a gateway fails every
...
class ZeroDayProvider(MultiProvider):
...
def configure_sdk_model_defaults(settings: Settings) -> None:
    """Apply ZeroDay config to SDK-native defaults."""
    llm = settings.llm
...
def _configure_litellm_compatibility() -> None:
    ...
    # ZeroDay uses LiteLLM's success callback to capture provider-reported cost.
...
def _install_openrouter_stream_cost_capture() -> None:
    ...
    lost and ZeroDay falls back to a cost-map estimate that is missing entirely
    ...
    from zeroday.report.state import streamed_openrouter_costs

    class _ZeroDayOpenRouterStreamingHandler(OpenRouterChatCompletionStreamingHandler):
    ...
    class _ZeroDayOpenrouterConfig(OpenrouterConfig):
        def get_model_response_iterator(
            self, streaming_response: Any, sync_stream: bool, json_mode: bool | None = False
        ) -> Any:
            return _ZeroDayOpenRouterStreamingHandler(
    ...
    litellm.OpenrouterConfig = _ZeroDayOpenrouterConfig  # type: ignore[misc]


OPENROUTER_ATTRIBUTION_HEADERS = {
    "HTTP-Referer": "https://zeroday.ai",
    "X-Title": "ZeroDay",
...
def _configure_extra_headers(llm: LlmSettings) -> None:
    ...
    regardless of the ``ZERODAY_LLM`` prefix.
    """
...
def _register_litellm_cost_callback() -> None:
    ...
    from zeroday.report.state import litellm_cost_callback
...
def supports_strict_tool_schemas(model_name: str) -> bool:
    """Return whether the route accepts strict tool schemas for ZeroDay's toolset.

    Claude caps a request at 20 strict tools and 16 union-typed parameters
    across all strict schemas. ZeroDay ships ~30 tools and the strict dialect
...
def routes_through_litellm(model_name: str | None) -> bool:
    """Whether :class:`ZeroDayProvider` sends this model through LiteLLM.
