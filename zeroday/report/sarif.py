"""SARIF 2.1.0 output for ZeroDay vulnerability reports.

Builds a GitHub code-scanning compatible SARIF document from ZeroDay findings
...
Design notes:
  * Rules are keyed on CWE (``id = CWE-NNN``), falling back to CVE, then
    ...
    ZeroDay output variants (``CWE-306``, ``cwe: 306``, ``306``) to the
    canonical ``CWE-NNN`` form so dedup works across runs.
  * SARIF only has three levels (error / warning / note). ZeroDay's five
    severities collapse into them. The raw severity label and CVSS score
    survive in ``result.properties.zeroday`` for downstream tools that can
...
SARIF_VERSION = "2.1.0"
TOOL_NAME = "ZeroDay"
TOOL_INFORMATION_URI = "https://zeroday.ai"
...
# SARIF only has three result levels; ZeroDay's five severities collapse here.
# Original label survives in ``result.properties.zeroday.severity``.
_SEVERITY_TO_LEVEL = {
    "critical": "error",
    "high": "error",
    "medium": "warning",
...
# zeroday-triage's DEFAULT_STRIDE_LEGS convention).
...
def build_sarif_report(
    ...
    *,
    ...
    :func:`zeroday.report.coverage.build_coverage_document`: its cleared
...
def _apply_repository_context(run: dict[str, Any], context: dict[str, Any]) -> None:
    ...
    ``automationDetails.id`` categorises the run (``zeroday/<owner>/<repo>``)
    so multiple ZeroDay runs against the same repo reconcile rather than pile
    ...
    if full_name:
        run["automationDetails"] = {"id": f"zeroday/{full_name}"}
...
def _build_rule(rule_id: str, report: dict[str, Any]) -> dict[str, Any]:
    """Build a SARIF rule descriptor from a ZeroDay finding."""
...
def _result_properties(
    ...
    *,
    ...
    """ZeroDay-specific metadata for downstream consumers.

    The top-level ``security-severity`` matches GitHub code-scanning's
    expected property. ZeroDay-specific fields are namespaced under
    ``zeroday`` so generic SARIF consumers don't see them by default.
    """
    properties: dict[str, Any] = {
        "security-severity": _security_severity(report),
    }
    if class_fingerprint:
        ...
        # filter alerts by it without parsing the nested zeroday.* tree.
        properties["zeroday_vuln_class_hash"] = class_fingerprint
    if is_synthetic:
        # Top-level so reviewers + downstream automation can filter
        # synthetic-anchored alerts without parsing the nested zeroday.*
        # tree.
        properties["synthetic_location"] = True

    zeroday: dict[str, Any] = {}
    for key in (
        "id",
        "severity",
        "cvss",
    ...
    if isinstance(dependency_metadata, dict) and dependency_metadata:
        zeroday["dependency_metadata"] = dependency_metadata
...
def _build_fixes(report: dict[str, Any]) -> list[dict[str, Any]] | None:
    ...
    ZeroDay findings carry the suggested change inline on each code
...
_COVERAGE_RULE_PREFIX = "zeroday-coverage"
...
def _rule_id(report: dict[str, Any]) -> str:
    ...
    CWE values are normalised from ZeroDay output variants (``CWE-306``,
    ...
    title = _string_value(report.get("title")) or "zeroday-finding"
    ...
    *,
    ...
    Computed from primitives that don't depend on LLM prose stability:
      ...
      - first SARIF location's URI + startLine — these come from
        ZeroDay's ``code_locations[].file`` and ``start_line`` which
    ...
    if uri:
        ...
        if start_line is not None:
            ...
            # line because ZeroDay code_locations carry the SINK line,
...
def _sarif_level(severity: Any) -> str:
    """Map ZeroDay severity labels to SARIF result levels."""
...
def _slugify(value: str) -> str:
    ...
    return slug or "zeroday-finding"
