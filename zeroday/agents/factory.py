"""Build SandboxAgents for root + child ZeroDay runs."""
...
from zeroday.tools.agents_graph.tools import (
    agent_finish,
    create_agent,
...
from zeroday.tools.coverage.tools import list_coverage, record_coverage, update_coverage
from zeroday.tools.finish.tool import finish_scan
from zeroday.tools.load_skill.tool import load_skill
from zeroday.tools.mcp import call_mcp, describe_mcp, list_mcps
from zeroday.tools.notes.tools import (
    create_note,
    delete_note,
    get_note,
    list_notes,
...
from zeroday.tools.nullish import is_nullish
from zeroday.tools.output_store import bound_and_store, bound_text
from zeroday.tools.proxy.tools import (
    list_requests,
    list_sitemap,
    repeat_request,
    scope_rules,
...
from zeroday.tools.reporting.tool import (
...
from zeroday.tools.respond.tool import respond_to_user
from zeroday.tools.thinking.tool import think
from zeroday.tools.threat_model.tools import (
    amend_threat_model,
    get_threat_model,
    save_threat_model,
)
from zeroday.tools.todo.tools import (
    create_todo,
    delete_todo,
    list_todos,
    mark_todo_done,
...
from zeroday.tools.web_search.tool import web_get_contents, web_search
...
def _with_bounded_result(tool: FunctionTool) -> FunctionTool:
    ...
    if getattr(tool, "_zeroday_bounded", False):
        return tool
    ...
    tool._zeroday_bounded = True  # type: ignore[attr-defined]
    return tool
...
def _with_coerced_arguments(tool: FunctionTool) -> FunctionTool:
    if getattr(tool, "_zeroday_coerced", False):
        return tool
    invoke_tool = tool.on_invoke_tool
    ...
    tool.on_invoke_tool = invoke
    tool._zeroday_coerced = True  # type: ignore[attr-defined]
    return tool
...
# ``zeroday.runtime.backends.register_backend``: register before the first
# ``build_zeroday_agent`` call and every agent (root + children) gets them.
