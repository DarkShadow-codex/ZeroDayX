...
class SuricataRuleGenerator:
    ...
    def generate_http_rule(
        ...
        rule_sid = sid or random.randint(9000000, 9999999)
        escaped_uri = uri_pattern.replace('"', '\\"')

        rule_text = f'alert http $EXTERNAL_NET any -> $HOME_NET any (msg:"ZERODAY {message}"; flow:established,to_server; http.uri; content:"{escaped_uri}"; nocase; classtype:web-application-attack; sid:{rule_sid}; rev:1;)'
