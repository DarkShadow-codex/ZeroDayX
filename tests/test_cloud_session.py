...
from zeroday.interface import cloud, platform_cli, platform_identity
from zeroday.interface.cloud import http
from zeroday.interface.cloud import session as cloud_session
...
def auth_path(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> Path:
    ...
    monkeypatch.delenv("ZERODAY_WORKSPACE_ID", raising=False)
    return path
...
def test_http_workspace_pin_is_captured_once(
    ...
    assert sent[0]["X-ZeroDay-Workspace"] == "org_start"
