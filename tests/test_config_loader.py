"""Tests for zeroday.config.loader: JSON overrides, alias resolution, persistence."""
...
from zeroday.config import loader
from zeroday.config.settings import ContextSettings
...
_LLM_ENV_KEYS = [
    ...
    "LITELLM_BASE_URL",
    "OLLAMA_API_BASE",
    "ZERODAY_REASONING_EFFORT",
    "ZERODAY_FORCE_REQUIRED_TOOL_CHOICE",
    "LLM_TIMEOUT",
    "PERPLEXITY_API_KEY",
    "EXA_API_KEY",
    "ZERODAY_WEB_SEARCH_PROVIDER",
    # RuntimeSettings
    "ZERODAY_IMAGE",
    "ZERODAY_RUNTIME_BACKEND",
    # TelemetrySettings
    "ZERODAY_TELEMETRY",
]
...
def test_read_json_overrides_maps_to_nested_settings(tmp_path: Path) -> None:
    path = tmp_path / "cli-config.json"
    path.write_text(
        json.dumps({"env": {"ZERODAY_LLM": "my-model", "PERPLEXITY_API_KEY": "pk"}}),
        encoding="utf-8",
    )
    assert loader._read_json_overrides(path) == {
        "llm": {"model": "my-model"},
...
def test_read_json_overrides_maps_exa_and_provider(tmp_path: Path) -> None:
    path = tmp_path / "cli-config.json"
    path.write_text(
        json.dumps({"env": {"EXA_API_KEY": "exa-key", "ZERODAY_WEB_SEARCH_PROVIDER": "exa"}}),
        encoding="utf-8",
    )
    assert loader._read_json_overrides(path) == {
...
def test_read_json_overrides_skips_keys_already_in_environ(
    ...
    monkeypatch.setenv("ZERODAY_LLM", "from-env")
    path = tmp_path / "cli-config.json"
    path.write_text(json.dumps({"env": {"ZERODAY_LLM": "from-file"}}), encoding="utf-8")
...
def test_tool_output_max_bytes_rejects_sub_notice_values() -> None:
    with pytest.raises(ValidationError):
        ContextSettings(ZERODAY_TOOL_OUTPUT_MAX_BYTES=64)


def test_tool_output_max_bytes_accepts_floor() -> None:
    assert ContextSettings(ZERODAY_TOOL_OUTPUT_MAX_BYTES=1024).tool_output_max_bytes == 1024
...
def test_apply_override_and_load_settings_round_trip(tmp_path: Path) -> None:
    path = tmp_path / "cli-config.json"
    path.write_text(
        json.dumps({"env": {"ZERODAY_LLM": "round-trip-model", "PERPLEXITY_API_KEY": "pk"}}),
        encoding="utf-8",
    )
...
def test_apply_config_override_invalidates_cache(tmp_path: Path) -> None:
    ...
    first.write_text(json.dumps({"env": {"ZERODAY_LLM": "first-model"}}), encoding="utf-8")
    second = tmp_path / "second.json"
    second.write_text(json.dumps({"env": {"ZERODAY_LLM": "second-model"}}), encoding="utf-8")
...
def test_persist_current_writes_env_block(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    assert json.loads(target.read_text(encoding="utf-8")) == {
        "env": {"ZERODAY_LLM": "persisted-model"}
    }
...
def test_persist_current_keeps_file_values_when_env_is_unset(tmp_path: Path) -> None:
    ...
    target.write_text(
        json.dumps({"env": {"ZERODAY_LLM": "file-model", "LLM_API_KEY": "file-key"}}),
        encoding="utf-8",
    )
    loader.apply_config_override(target)
    ...
    assert json.loads(target.read_text(encoding="utf-8")) == {
        "env": {"ZERODAY_LLM": "file-model", "LLM_API_KEY": "file-key"}
    }
...
def test_persist_current_env_overrides_file_value(
    ...
    target = tmp_path / "cli-config.json"
    target.write_text(
        json.dumps({"env": {"ZERODAY_LLM": "file-model", "PERPLEXITY_API_KEY": "file-pplx"}}),
        encoding="utf-8",
    )
    loader.apply_config_override(target)
    ...
    assert json.loads(target.read_text(encoding="utf-8")) == {
        "env": {"ZERODAY_LLM": "file-model", "PERPLEXITY_API_KEY": "env-pplx"}
    }
...
def test_linked_llm_model_change_drops_stored_key_and_base(
    ...
    loader.apply_config_override(target)
    monkeypatch.setenv("ZERODAY_LLM", "env-model")
    ...
    assert json.loads(target.read_text(encoding="utf-8")) == {
        "env": {"ZERODAY_LLM": "env-model", "PERPLEXITY_API_KEY": "pplx"}
    }
...
def test_linked_llm_unchanged_env_keeps_stored_key(
    ...
    loader.apply_config_override(target)
    monkeypatch.setenv("ZERODAY_LLM", "file-model")
...
def test_persist_current_empty_env_clears_file_value(
    ...
    target = tmp_path / "cli-config.json"
    target.write_text(
        json.dumps({"env": {"ZERODAY_LLM": "file-model", "PERPLEXITY_API_KEY": "pplx"}}),
        encoding="utf-8",
    )
    loader.apply_config_override(target)
    ...
    assert json.loads(target.read_text(encoding="utf-8")) == {"env": {"ZERODAY_LLM": "file-model"}}
...
def test_persist_current_replaces_corrupt_file(
    ...
    assert json.loads(target.read_text(encoding="utf-8")) == {"env": {"ZERODAY_LLM": "env-model"}}
