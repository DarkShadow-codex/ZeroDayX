...
def test_per_call_extra_tools_stack_with_registry() -> None:
    ...
    agent = factory.build_zeroday_agent(is_root=True, extra_tools=[_tool("per_call")])
...
def test_per_call_extra_tools_reject_duplicate_registered_names() -> None:
    ...
    with pytest.raises(ValueError, match="same_name"):
        factory.build_zeroday_agent(is_root=True, extra_tools=[_tool("same_name")])


def test_instructions_override_is_used_verbatim() -> None:
    custom = "You are a scan agent. Follow the provided scope."

    agent = factory.build_zeroday_agent(is_root=True, instructions_override=custom)

    assert agent.instructions == custom


def test_no_override_renders_builtin_prompt() -> None:
    agent = factory.build_zeroday_agent(is_root=True)
...
def test_respond_to_user_is_interactive_only() -> None:
    ...
    interactive = factory.build_zeroday_agent(is_root=True, interactive=True)
    autonomous = factory.build_zeroday_agent(is_root=True, interactive=False)
...
def test_wait_for_agents_is_available_in_both_modes() -> None:
    for interactive in (True, False):
        agent = factory.build_zeroday_agent(is_root=True, interactive=interactive)
...
def test_strict_tool_schemas_can_be_disabled_per_route() -> None:
    ...
    agent = factory.build_zeroday_agent(is_root=True, strict_tool_schemas=False)
...
def test_disabling_strict_leaves_shared_tools_untouched() -> None:
    factory.build_zeroday_agent(is_root=True, strict_tool_schemas=False)
    agent = factory.build_zeroday_agent(is_root=True)
