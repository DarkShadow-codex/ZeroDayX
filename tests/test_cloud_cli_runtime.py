...
from zeroday.interface.cloud import http, render, runner, source_scan
from zeroday.interface.cloud.source_upload import prepare_source
from zeroday.interface.main import main as interface_main
...
def _token(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setenv("ZERODAY_API_TOKEN", "runtime-test-token")
            ...
            "items": [
                {
                    ...
                    "repositories": [
                        {
                            "url": "https://github.com/usezeroday/zeroday",
                            "branch": "feature",
                            "provider": "github",
                        }
                    ],
...
def test_connector_enrollment_command_is_complete_multiline_and_terminal_safe(
    ...
    command = (
        ...
        "  ghcr.io/usezeroday/connector:latest"
    )
    ...
    assert "ghcr.io/usezeroday/connector:latest" in output
...
def test_pr_detail_summarizes_nested_findings() -> None:
    ...
    assert "usezeroday/zeroday" in output
...
def test_redirected_session_argument_errors_are_json_only(command: str, capsys: Any) -> None:
    ...
    captured = capsys.readouterr()
    payload = json.loads(captured.out)
    assert f"zeroday cloud {command}" in payload["error"]
    assert captured.err == ""
...
def test_session_help_is_specific_and_human_whoami_shows_scopes(
    ...
    assert "zeroday cloud whoami" in who_help
...
def test_login_workspace_selector_prefers_ids_and_rejects_duplicate_names() -> None:
    ...
    organizations = [
        {"id": "org_1", "name": "org_2"},
        {"id": "org_2", "name": "ZeroDay"},
        {"id": "org_3", "name": "ZeroDay"},
    ]
    ...
    with pytest.raises(platform_cli.PlatformAuthError, match="org_2, org_3"):
        platform_cli._choose_workspace(console, organizations, "ZeroDay")
...
def test_device_flow_accepts_external_authkit_url_and_binds_token_origin(
    ...
    monkeypatch.setattr(platform_cli, "_app_url", lambda: "https://preview.zeroday.ai")
    ...
    assert record["app_url"] == "https://preview.zeroday.ai"
...
def test_missing_verb_json_is_structured(capsys: Any) -> None:
    ...
    payload = json.loads(capsys.readouterr().out)
    assert payload["command"] == "zeroday cloud uploads"
...
def test_root_help_accepts_json_before_help_and_leaf_help_stays_specific(
    ...
    assert json.loads(capsys.readouterr().out)["command"] == "zeroday cloud"

    assert cloud.run_cloud(["scans", "get", "scan-1", "-h"]) == 0
    leaf_help = capsys.readouterr().out
    assert "zeroday cloud scans get" in leaf_help
...
def test_non_tty_dispatcher_always_emits_structured_json(
    ...
    assert json.loads(capsys.readouterr().out)["command"] == "zeroday cloud uploads"
    ...
    payload = json.loads(capsys.readouterr().out)
    assert payload["command"] == "zeroday cloud scans"
...
@pytest.mark.parametrize(
    ...
    [
        ...
        "https://app.zeroday.ai@127.0.0.1/storage/v1/object/upload/sign/bucket/file",
...
@pytest.mark.parametrize(
    ...
    [
        (
            "https://app.zeroday.ai",
            "https://project-ref.supabase.co/storage/v1/object/upload/sign/bucket/file?token=signed%2Fvalue",
        ),
        (
            "https://zeroday.corp.internal",
            "https://zeroday.corp.internal/storage/v1/object/upload/sign/bucket/file",
        ),
        (
            "http://127.0.0.1:3000",
...
def test_one_time_api_token_has_save_now_warning(
    ...
    monkeypatch.setattr(
        http,
        "request",
        lambda *_a, **_k: FakeResponse({"id": "token-1", "token": "zeroday_pat_once"}),
    )
...
def test_root_help_advertises_cloud_and_completions(
    ...
    monkeypatch.setattr(sys, "argv", ["zeroday", "--help"])
    with pytest.raises(SystemExit) as raised:
        interface_main()
    assert raised.value.code == 0
    output = capsys.readouterr().out
    assert "zeroday cloud" in output
    assert "zeroday completions" in output
