...
from zeroday.interface import cloud
from zeroday.interface.cloud import http, source_upload
...
def test_zerodayignore_and_cli_excludes_are_applied(tmp_path: Path) -> None:
    (tmp_path / "keep.py").write_text("keep\n", encoding="utf-8")
    (tmp_path / "generated.py").write_text("generated\n", encoding="utf-8")
    (tmp_path / "test_app.py").write_text("test\n", encoding="utf-8")
    (tmp_path / ".zerodayignore").write_text("generated.py\n", encoding="utf-8")
...
def test_zerodayignore_trailing_slash_excludes_the_whole_directory(tmp_path: Path) -> None:
    (tmp_path / "keep.py").write_text("keep\n", encoding="utf-8")
    ...
    (tmp_path / ".zerodayignore").write_text("private/\n", encoding="utf-8")
...
def test_zerodayignore_size_and_pattern_counts_are_bounded(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    ignore = tmp_path / ".zerodayignore"
...
def test_zerodayignore_must_be_a_nonblocking_regular_file(tmp_path: Path) -> None:
    (tmp_path / "app.py").write_text("print('ok')\n", encoding="utf-8")
    os.mkfifo(tmp_path / ".zerodayignore")
...
def test_oversize_archive_names_largest_files_and_exclude_guidance(
    ...
    assert "--dry-run --show-files" in raised.value.next_step
    assert not list(tmp_path.glob("zeroday-source-*.zip"))
