...
from zeroday.agents.factory import _BASE_TOOLS
from zeroday.tools.threat_model import tools as threat_model_tools
...
def test_saved_model_round_trips(tmp_path: Path) -> None:
    ...
    assert _save_impl(str(repo), _MODEL, "ZeroDay")["success"] is True
    result = _get_impl(str(repo))
