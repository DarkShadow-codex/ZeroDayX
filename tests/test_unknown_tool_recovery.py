...
Models regularly invent tool names that ZeroDay does not register (``read_file``
is a common one, borrowed from other agent frameworks). The SDK's default is to
raise ``ModelBehaviorError``, which ends the whole run: nothing in ZeroDay retries
...
from zeroday.config.models import _NonStreamingModel
...
def _agent() -> Agent[Any]:
    ...
    return Agent(name="ZeroDay", instructions="test", tools=[real_tool], model="gw-model")
