...
from zeroday.config.settings import IntegrationSettings
from zeroday.interface.environment import _missing_web_search_vars
from zeroday.tools.web_search import tool
...
def test_explicit_exa_ignores_a_configured_perplexity_key() -> None:
    integrations = IntegrationSettings(
        ...
        EXA_API_KEY="ek",
        ZERODAY_WEB_SEARCH_PROVIDER="exa",
    )
    assert tool._resolve_provider(integrations) == ("exa", "ek")
...
def test_explicit_perplexity_ignores_a_configured_exa_key() -> None:
    integrations = IntegrationSettings(
        PERPLEXITY_API_KEY="pk",
        EXA_API_KEY="ek",
        ZERODAY_WEB_SEARCH_PROVIDER="perplexity",
    )
    assert tool._resolve_provider(integrations) == ("perplexity", "pk")
...
def test_exa_search_type_and_num_results_are_configurable(
    ...
    class _Settings:
        integrations = IntegrationSettings(
            EXA_API_KEY="ek",
            ZERODAY_EXA_SEARCH_TYPE="deep-reasoning",
            ZERODAY_EXA_NUM_RESULTS=3,
        )
