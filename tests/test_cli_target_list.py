...
def test_parse_arguments_accepts_target_list_file(
    ...
    monkeypatch.setattr(sys, "argv", ["zeroday", "--target-list", str(target_list), "-n"])
...
def test_parse_arguments_combines_target_and_target_list(
    ...
    monkeypatch.setattr(
        sys,
        "argv",
        ["zeroday", "-t", "https://test1.com/", "--target-list", str(target_list)],
    )
...
def test_parse_arguments_rejects_resume_with_target_list(
    ...
    monkeypatch.setattr(
        sys,
        "argv",
        ["zeroday", "--resume", "old-run", "--target-list", str(target_list)],
    )
...
def test_resume_restores_a_target_less_workspace_mount(
    ...
    monkeypatch.setattr(sys, "argv", ["zeroday", "--resume", "pentest_abcd"])
...
def test_resume_non_object_run_json_exits(
    ...
    monkeypatch.chdir(tmp_path)
    run_dir = tmp_path / "zeroday_runs" / "pentest_abcd"
