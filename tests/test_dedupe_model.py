...
from zeroday.config.settings import DedupeSettings
from zeroday.report.dedupe import _dedupe_model_settings, resolve_dedupe_model
...
def test_dedupe_key_bound_to_model_client_not_global_env() -> None:
    dedupe = DedupeSettings(ZERODAY_DEDUPE_MODEL="deepseek/cheap", DEDUPE_LLM_API_KEY="dedupe-key")
...
def test_dedupe_settings_carry_no_request_credentials() -> None:
    dedupe = DedupeSettings(
        ZERODAY_DEDUPE_MODEL="deepseek/cheap",
...
def test_dedupe_endpoint_bound_to_model_client() -> None:
    dedupe = DedupeSettings(
        ZERODAY_DEDUPE_MODEL="openai/cheap",
        DEDUPE_LLM_API_KEY="dedupe-key",
        DEDUPE_LLM_API_BASE="https://dedupe.example/v1",
    )
...
def test_dedupe_without_credentials_uses_default_provider() -> None:
    dedupe = DedupeSettings(ZERODAY_DEDUPE_MODEL="deepseek/cheap")
    model = _unwrap(resolve_dedupe_model(dedupe, "deepseek/cheap"))
...
def test_dedupe_model_read_from_env(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setenv("ZERODAY_DEDUPE_MODEL", "deepseek/deepseek-v4-flash")
    monkeypatch.setenv("ZERODAY_DEDUPE_REASONING_EFFORT", "low")
...
def test_config_file_loads_dedupe_model(
    ...
    for key in (
        ...
        "OPENAI_API_KEY",
        "LLM_API_BASE",
        "ZERODAY_DEDUPE_MODEL",
        "ZERODAY_DEDUPE_REASONING_EFFORT",
    ):
    ...
    path.write_text(
        json.dumps(
            {
                "env": {
                    "ZERODAY_LLM": "openai/root",
                    "ZERODAY_DEDUPE_MODEL": "deepseek/cheap",
                    "ZERODAY_DEDUPE_REASONING_EFFORT": "minimal",
                }
            }
        ),
        encoding="utf-8",
