...
from zeroday.config import codex, loader
from zeroday.config.loader import load_settings
from zeroday.config.models import ZeroDayProvider, _NonStreamingModel, _TurnGuardModel
...
def _reset_settings(monkeypatch: pytest.MonkeyPatch) -> Iterator[None]:
    for key in ("ZERODAY_LLM", "LLM_DISABLE_STREAMING"):
...
def test_get_model_wraps_when_disabled(
    ...
    inner = _DummyModel()
    monkeypatch.setattr("zeroday.config.models.MultiProvider.get_model", lambda *_: inner)
    monkeypatch.setenv("LLM_DISABLE_STREAMING", "true")
    load_settings()

    model = ZeroDayProvider().get_model("openai/gpt-4o-mini")
...
def test_get_model_guards_subscription_model_but_keeps_it_streaming(
    ...
    model = ZeroDayProvider().get_model("gpt-5.5")
    assert isinstance(model, _TurnGuardModel)
