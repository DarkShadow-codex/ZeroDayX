"""Tests for the `zeroday cloud` CLI: routing, request building, and output."""
...
from zeroday.interface import cloud, platform_cli
from zeroday.interface.cloud import http, render, runner, workspaces
from zeroday.interface.cloud.spec import GROUP_HELP, SPEC
...
def _token_env(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setenv("ZERODAY_API_TOKEN", "test-token")
...
def test_successful_html_response_is_reported_without_dumping_html(
    ...
    assert "ZERODAY_APP_URL" in output
...
def test_placeholder_substitution_percent_encodes_path_segments(
    ...
    monkeypatch.setattr(http, "request", fake_request)
    code = cloud.run_cloud(["knowledge", "repos", "entries", "usezeroday/.github", "--json"])
    assert code == 0
    assert seen["path"] == "/knowledge/repos/usezeroday%2F.github/entries"
...
def test_token_create_accepts_expiry_and_rbac_scope_flags(
    ...
    def fake_request(_method: str, _path: str, **kwargs: Any) -> FakeResponse:
        seen["body"] = kwargs.get("body")
        return FakeResponse(payload={"id": "token-1", "token": "zeroday_pat_once"})
...
def test_provider_token_does_not_override_zeroday_api_auth(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    seen: dict[str, Any] = {}
...
def test_insufficient_credits_always_prints_topup_instruction(
    ...
    argv = ["scans", "start", "--domain-ids", "d1", "--app-url", "https://app.zeroday.ai"]
    assert cloud.run_cloud(argv) == http.EXIT_PAYMENT
    output = " ".join(capsys.readouterr().out.split())
    assert "Error: Out of credits." in output
    assert "Next step:" in output
    assert "zeroday cloud billing topup --credits <count>" in output
    assert "https://app.zeroday.ai/settings/billing" in output
    assert "zeroday cloud billing credits" in output


def test_insufficient_credits_shows_platform_hint_once(
    monkeypatch: pytest.MonkeyPatch, capsys: Any
) -> None:
    hint = "Buy credits at https://app.zeroday.ai/settings/billing. Then retry this request."
    payload = {
        "detail": f"Out of credits. {hint}",
        "code": "scan_credit_limit_reached",
        "hint": hint,
        "topup_url": "https://app.zeroday.ai/settings/billing",
    }
    monkeypatch.setattr(
    ...
    assert result["topup_url"] == "https://app.zeroday.ai/settings/billing"
...
def test_payment_required_without_body_names_the_topup_command(
    ...
    argv = ["scans", "start", "--domain-ids", "d1", "--json", "--app-url", "https://app.zeroday.ai"]
    assert cloud.run_cloud(argv) == http.EXIT_PAYMENT
    result = json.loads(capsys.readouterr().out)
    assert result["error"] == "Not enough credits to run this command."
    assert "zeroday cloud billing topup --credits <count>" in result["next_step"]
    assert "https://app.zeroday.ai/settings/billing" in result["next_step"]
...
def test_missing_token_exits_with_auth_code(
    ...
    monkeypatch.delenv("ZERODAY_API_TOKEN", raising=False)
...
def test_stored_token_is_never_sent_to_a_different_platform_origin(
    ...
    monkeypatch.setattr(
        http,
        "read_record",
        lambda: {"api_token": "stored-secret", "app_url": "https://app.zeroday.ai"},
    )
    monkeypatch.setattr(
        requests,
        "request",
...
def test_stored_token_requires_an_issuer_binding(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.delenv("ZERODAY_API_TOKEN", raising=False)
    monkeypatch.setattr(http, "_app_url_override", "https://app.zeroday.ai")
...
def test_stored_token_is_sent_only_to_its_bound_platform(
    ...
    monkeypatch.delenv("ZERODAY_API_TOKEN", raising=False)
    monkeypatch.setattr(http, "_app_url_override", "https://preview.zeroday.ai")
    monkeypatch.setattr(
        http,
        "read_record",
        lambda: {"api_token": "stored-secret", "app_url": "https://preview.zeroday.ai"},
    )
    seen: dict[str, Any] = {}
    ...
    assert seen["url"] == "https://preview.zeroday.ai/api/v1/billing/credits"
...
def test_topup_keeps_token_out_of_wallet_process_and_forwards_payment(
    ...
    assert child_envs[0].get("ZERODAY_API_TOKEN") is None
    ...
    assert upstream["headers"]["X-ZeroDay-Authorization"] == f"Bearer {api_credential}"
...
def test_every_command_builds_a_parser() -> None:
    for group, commands in SPEC.items():
        for verb, cmd in commands.items():
            ...
            assert parser.prog == f"zeroday cloud {group} {verb}"
...
def test_billing_subscribe_prints_checkout_url(
    ...
    monkeypatch.setattr(http, "request", fake_request)
    code = cloud.run_cloud(["billing", "subscribe", "--plan", "zeroday_cloud", "--json"])
    assert code == 0
    assert seen["method"] == "POST"
    assert seen["path"] == "/billing/checkout"
    assert seen["body"] == {"product": "zeroday_cloud"}
...
@pytest.mark.parametrize(
    ...
    [
        (
            ["billing", "subscribe", "--plan", "zeroday_cloud"],
...
def test_workspace_use_environment_token_starts_with_fresh_account_state(
    ...
    monkeypatch.setattr(platform_cli, "AUTH_PATH", auth_path)
    monkeypatch.setattr(workspaces, "AUTH_PATH", auth_path)
    monkeypatch.setenv("ZERODAY_API_TOKEN", "account-b-token")
    platform_cli.save_record(
        {
            "api_token": "account-a-token",
            "email": "account-a@example.test",
...
def test_integrations_human_list_exposes_installation_id_and_json_stays_full(
    ...
    output = capsys.readouterr().out
    for value in ("1.", "2.", "github", "usezeroday", "154419799", "jira", "Security"):
                ...
                "items": [
                    {
                        ...
                        "user_id": "user-id",
                        "installation_id": 42,
                        "repository_full_name": "usezeroday/zeroday",
                        "pr_number": 1177,
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            ...
            ("usezeroday/zeroday", "github", "yes", "repo-id"),
            ("org-secret", "user-secret", "added by"),
        ),
        (
            ["knowledge", "list"],
        ...
        (
                ...
                "items": [
                    {
                        ...
                        "mfa_email": "inbox@security-mail.zeroday.ai",
                        "has_password": True,
            ...
            (
                ...
                "admin@example.com",
                "email_otp",
                "inbox@security-mail.zeroday.ai",
                "test-user-id",
            ),
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            ...
            {
                "address": "inbox@security-mail.zeroday.ai",
                "messages": [
                    {
                        "id": "message-id",
            ...
            (
                ...
                "123456",
                "message-id",
                "Inbox: inbox@security-mail.zeroday.ai",
            ),
            (),
        ),
        (
            ["knowledge", "repos", "entries", "usezeroday/zeroday"],
            {
                "organization_id": "org-secret",
                "repo_key": "usezeroday/zeroday",
                ...
                "docs": [
                    {
                        ...
                        "title": "Auth notes",
                        "source_type": "system",
                        "source_id": "repos/usezeroday__zeroday/auth.md",
                        "tags": [],
            ...
            (
                "Auth notes",
                "system",
                "repos/usezeroday__zeroday/auth.md",
                "doc-id",
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            ...
            ("usezeroday/zeroday", "4", "2026-08-27"),
            ("future_internal_field", "hidden"),
        ),
        (
            ["audit", "list"],
...
def test_wide_knowledge_table_keeps_title_readable_with_long_identifiers() -> None:
    ...
    source_id = "repos/usezeroday__zeroday/" + "nested/" * 12 + "authentication.md"
...
def test_human_get_prioritizes_details_and_hides_internal_identity_fields(
    ...
    output = capsys.readouterr().out
    for value in ("usezeroday/zeroday", "1177", "Improve cloud CLI", "pass", "Example"):
        assert value in output
...
def test_logout_help_does_not_remove_stored_auth(
    ...
    assert "usage: zeroday cloud logout" in capsys.readouterr().out
