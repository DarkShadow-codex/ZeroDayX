"""Tests for the SARIF 2.1.0 emitter in zeroday.report.sarif."""
...
def test_write_sarif_basic_shape(tmp_path: Path) -> None:
    ...
    run = doc["runs"][0]
    assert run["tool"]["driver"]["name"] == "ZeroDay"
...
def test_write_sarif_never_embeds_poc_script(tmp_path: Path) -> None:
    ...
    poc = _read(tmp_path)["runs"][0]["results"][0]["properties"]["zeroday"]["poc"]
...
def test_write_sarif_emits_version_control_provenance(tmp_path: Path) -> None:
    ...
    assert run["automationDetails"] == {"id": "zeroday/acme/widget"}
...
def test_coverage_results_declare_their_own_rules(tmp_path: Path) -> None:
    ...
    coverage_rules = [rule for rule in rules if rule["id"].startswith("zeroday-coverage/")]
...
def test_calibration_metadata_survives_into_result_properties(tmp_path: Path) -> None:
    ...
    zeroday = _read(tmp_path)["runs"][0]["results"][0]["properties"]["zeroday"]

    assert zeroday["confidence"] == "medium"
    assert zeroday["counterevidence"] == "WAF blocks the naive payload."
    assert zeroday["confidence_rationale"] == "Reproduced once out of three attempts."
    assert zeroday["severity_change_conditions"] == "Critical if the WAF rule is removed."
    assert zeroday["fix_verification"] == "Not retested."
