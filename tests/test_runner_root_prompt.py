"""Tests for root scan prompt options in run_zeroday_scan.

Verify that ``root_instructions_override`` and ``extra_system_prompt_context``
flow through to the root agent's ``build_zeroday_agent`` call.
"""
...
import zeroday.tools.mcp as mcp_pkg
import zeroday.tools.notes.tools as notes_tools
import zeroday.tools.todo.tools as todo_tools
from zeroday.core import runner
from zeroday.core.agents import AgentCoordinator
from zeroday.runtime import session_manager
from zeroday.tools.mcp import BearerAuth, McpConnectionConfig, McpConnectionRequest
...
def _patch_engine_scaffold(
    ...
    """Stub out everything around build_zeroday_agent and stop at run_agent_loop.

    Returns a dict that will be populated with the kwargs the runner passed to
    ``build_zeroday_agent`` for the root agent.
    """
    ...
    def _build_zeroday_agent(**kwargs: Any) -> object:
        if kwargs.get("is_root") and "kwargs" not in captured:
            captured["kwargs"] = kwargs
        return object()

    monkeypatch.setattr(runner, "build_zeroday_agent", _build_zeroday_agent)
...
async def test_root_prompt_options_flow_into_root_agent(
    ...
    scope_context = {
        ...
        "authorization_source": "zeroday_platform_verified_targets",
        "authorized_targets": [
            {
