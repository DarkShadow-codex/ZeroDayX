...
def _isolate_mcp_config(
    ...
    ``run_zeroday_scan`` connects the MCP servers listed in
    ``~/.zeroday/mcp-servers.json`` and threads an inventory of them into the
    ...
    ``ZERODAY_MCP_CONFIG`` after this runs and so override it.
    """
    missing = tmp_path_factory.mktemp("mcp-isolation") / "no-servers.json"
    monkeypatch.setenv("ZERODAY_MCP_CONFIG", str(missing))
...
def _isolate_wallet_config(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    ``zeroday cloud billing topup`` chooses the Stripe Link flow or the
