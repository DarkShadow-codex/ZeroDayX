...
when it does not, the runner reads ``~/.zeroday/mcp-servers.json`` itself and wraps
...
from zeroday.tools.mcp import McpConnectionConfig, McpConnectionRequest
...
def _wire_runner(monkeypatch: pytest.MonkeyPatch, tmp_path: Any) -> None:
    ...
    monkeypatch.setattr(runner, "build_zeroday_agent", lambda **_k: object())
