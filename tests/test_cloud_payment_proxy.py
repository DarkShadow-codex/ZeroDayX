...
from zeroday.interface.cloud import payment_proxy
...
def test_bridge_bounds_decompressed_upstream_response(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    with payment_proxy.wallet_payment_bridge(
        ...
        api_token="zeroday-secret",  # noqa: S106
        expected_body=b"{}",
    ) as wallet_url:
        request = urllib.request.Request(  # noqa: S310
            wallet_url,
    ...
    with payment_proxy.wallet_payment_bridge(
        ...
        result = _post(
            ...
            b'{"credits":5}',
            {
                ...
                "X-ZeroDay-Authorization": "Bearer attacker",
                "X-ZeroDay-Workspace": "org_attacker",
            },
        )
        ...
        assert headers["X-ZeroDay-Authorization"] == "Bearer zeroday-secret"
        assert headers["X-ZeroDay-Workspace"] == "org_trusted"
