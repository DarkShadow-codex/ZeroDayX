...
from zeroday.agents import factory
from zeroday.agents.prompt import render_system_prompt
from zeroday.interface.tui.live_view import TuiLiveView
...
from zeroday.tools.mcp import client as mcp_client
from zeroday.tools.mcp import session as mcp_session_mod
...
def _clear_mcp_env(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    for name in ("ZERODAY_MCP_CONFIG", "ZERODAY_MCP_ONLY", "ZERODAY_MCP_EXCLUDE"):
...
def test_agent_carries_exactly_the_dispatch_tools_regardless_of_connections() -> None:
    ...
    provider tool."""
    root = factory.build_zeroday_agent(is_root=True)
    child = factory.build_zeroday_agent(is_root=False)
    ...
    # contribute tools.
    assert root_names == [t.name for t in factory.build_zeroday_agent(is_root=True).tools]
...
def test_loader_reads_env_var_override(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setenv("ZERODAY_MCP_CONFIG", str(config_file))
...
def test_loader_include_selection_keeps_only_named(
    ...
    monkeypatch.setenv("ZERODAY_MCP_ONLY", "a,c")
...
def test_loader_exclude_selection_drops_named(
    ...
    config_file = _names_file(tmp_path, "a", "b", "c")
    monkeypatch.setenv("ZERODAY_MCP_EXCLUDE", "b")
...
# namespaced_tool_name stays in zeroday.tools.mcp.naming so call_mcp can build the
...
async def test_reconnect_reuses_the_stored_config_and_never_logs_the_token(
    ...
    with caplog.at_level("DEBUG", logger="zeroday.tools.mcp.session"):
        out = await call_mcp.on_invoke_tool(
            _ctx(registry), json.dumps({"connection": "fs", "tool": "read_file"})
        )
...
async def test_session_on_dead_fires_once_on_the_death_transition(
    ...
    clock = [100.0]
    monkeypatch.setattr("zeroday.tools.mcp.session.time.monotonic", lambda: clock[0])
