...
each other's import locks (warm-up: ``zeroday.core.runner`` -> ``agents``;
...
from zeroday.llm import warmup
...
def test_zeroday_report_does_not_import_the_agents_graph() -> None:
    result = _run(
        """
        import sys

        import zeroday.report

        agents_modules = [m for m in sys.modules if m == "agents" or m.startswith("agents.")]
        assert not agents_modules, agents_modules
        assert "zeroday.report.dedupe" not in sys.modules
        """
    )
...
def test_check_duplicate_resolves_lazily() -> None:
    result = _run(
        """
        import zeroday.report
        from zeroday.report import check_duplicate
        from zeroday.report.dedupe import check_duplicate as direct

        assert zeroday.report.check_duplicate is direct is check_duplicate
        """
    )
    assert result.returncode == 0, result.stderr
...
def test_wait_for_import_warmup_lets_main_thread_import_the_agents_graph() -> None:
    result = _run(
        ...
        assert "zeroday.core.runner" in sys.modules
        """
    )
    assert result.returncode == 0, result.stderr
...
def test_failed_warm_import_does_not_raise() -> None:
    warmup._warm(("zeroday_no_such_module_for_warmup_test",))
