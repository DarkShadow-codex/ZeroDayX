...
cli_main: Any = importlib.import_module("zeroday.interface.main")
...
def test_mcp_config_flag_sets_loader_override(
    ...
    monkeypatch.delenv("ZERODAY_MCP_CONFIG", raising=False)
    monkeypatch.setattr(
        sys, "argv", ["zeroday", "-t", "https://test.com/", "-n", "--mcp-config", str(config)]
    )

    args = cli_main.parse_arguments()

    assert args.mcp_config == str(config)
    assert os.environ["ZERODAY_MCP_CONFIG"] == str(config)
...
def test_mcp_config_flag_rejects_missing_file(
    ...
    monkeypatch.setattr(
        sys, "argv", ["zeroday", "-t", "https://test.com/", "-n", "--mcp-config", str(missing)]
    )
...
def test_mcp_server_flags_set_selection_env(monkeypatch: pytest.MonkeyPatch) -> None:
    _stub_settings(monkeypatch)
    monkeypatch.delenv("ZERODAY_MCP_ONLY", raising=False)
    monkeypatch.delenv("ZERODAY_MCP_EXCLUDE", raising=False)
    monkeypatch.setattr(
        sys,
        "argv",
        [
    ...
    assert os.environ["ZERODAY_MCP_ONLY"] == "a,b"
    assert os.environ["ZERODAY_MCP_EXCLUDE"] == "c"
