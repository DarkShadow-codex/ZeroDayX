...
from zeroday.config import ContextSettings
from zeroday.llm import compaction
...
def _patch_summary(
    ...
    monkeypatch.setattr(compaction, "ZeroDayProvider", FakeProvider)
...
async def test_maybe_compact_skips_when_summary_fails(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(compaction, "ZeroDayProvider", BoomProvider)
    session = FakeSession(_turns(12))
    before = await session.get_items()
